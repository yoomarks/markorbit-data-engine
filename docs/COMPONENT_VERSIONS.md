...
| Trademark country scaffold | `TRADEMARK_COUNTRY_SCAFFOLD_V4` | Generates framework-aligned country packages; HTTP_API sources receive a reusable paginated HTTP acquisition skeleton while generated sources remain disabled. |
...
18. **Country Scaffold V4 is transport-aware but evidence-conservative.** HTTP sources receive reusable transport/pagination imports and explicit runtime/interpretation hooks, but no endpoint, credential, pagination or termination semantics are guessed.
