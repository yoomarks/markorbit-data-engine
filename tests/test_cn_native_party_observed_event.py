...
class _Client:
    ...
    def command(self, sql, *args, **kwargs):
        ...
        if self.fail_command is not None and len(self.commands) == self.fail_command:
            raise RuntimeError("synthetic native party-observed-event failure")
...
def test_party_observed_event_sql_preserves_baseline_delta_and_hash_semantics() -> None:
    package = uuid.uuid4()
    sql = party_observed_event_sql(
        package,
        package_kind="MONTHLY_PATCH",
        source_effective_date=date(2026, 3, 31),
        source_rank=700,
        lower="A100",
        upper="A200",
    )

    assert "concat(incoming.role, '_RELATION_OBSERVED')" in sql
    assert "toDate32('2026-03-31')" in sql
    assert "'PARTY'" in sql
    assert "lowerUTF8(incoming.role)" in sql
    assert "'entity_id', ifNull(toString(incoming.entity_id), '')" in sql
    assert "'OFFICIAL_FACT_OBSERVATION'" in sql
    assert "LEFT JOIN" in sql
    assert "FROM markorbit_facts.cn_case_party_current FINAL" in sql
    assert "(application_number, role, relation_key) IN" in sql
    assert "cur.application_number = '' OR cur.source_rank < 700" in sql
    assert "cur.application_number = ''" in sql
    ...
    assert "incoming.application_number, '|', incoming.role, '|OBSERVED|'" in sql
    assert "toString(700)" in sql


def test_party_observed_event_resume_skips_successful_range() -> None:
    package = uuid.uuid4()
    store = _Store()
    first_client = _Client(query_results=[[("A300",)], []], fail_command=2)
    first = NativePartyObservedEventExecutor(
        client=first_client,
    ...
    with pytest.raises(RuntimeError, match=r"task=2/2.*synthetic native party-observed-event failure"):
        first.execute()

    retry_client = _Client(query_results=[[("A300",)], []])
    retry = NativePartyObservedEventExecutor(
        client=retry_client,
        package_uuid=package,
        package_kind="MONTHLY_PATCH",
        source_effective_date=date(2026, 3, 31),
...
def test_fresh_party_observed_event_cutover_bypasses_delegate(monkeypatch) -> None:
    package = uuid.uuid4()
    store = _Store()
    delegate = _Delegate()
    execution = _Client(query_results=[[]])
    monkeypatch.setattr(
        native_party_observed_event,
        "get_package",
        lambda package_id: {
            "package_kind": "MONTHLY_PATCH",
    ...
    client = NativePartyObservedEventCutoverClient(
        delegate,
    ...
    assert client.native_party_observed_event_enabled is True
...
def test_superseded_shape_is_not_claimed_by_party_observed_cutover() -> None:
    package = uuid.uuid4()
    store = _Store()
    delegate = _Delegate()
    client = NativePartyObservedEventCutoverClient(
...
def test_old_checkpoint_without_marker_keeps_legacy_party_observed_event() -> None:
    package = uuid.uuid4()
    delegate = _Delegate()
    client = NativePartyObservedEventCutoverClient(
        delegate,
    ...
    assert client.native_party_observed_event_enabled is False
