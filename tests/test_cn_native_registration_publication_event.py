...
class _Client:
    ...
    def command(self, sql, *args, **kwargs):
        ...
        if self.fail_command is not None and len(self.commands) == self.fail_command:
            raise RuntimeError("synthetic registration-publication failure")
...
def test_registration_sql_preserves_delta_payload_and_range() -> None:
    package = uuid.uuid4()
    sql = registration_publication_event_sql(
        package,
    ...
    assert "incoming.registration_pub_date" in sql
    assert "'registration_publication'" in sql
    assert "toJSONString(map('date'" in sql
    assert "cur.source_rank < 700" in sql
    assert "incoming.registration_pub_date IS NOT NULL" in sql
    assert "cur.registration_pub_issue" in sql
    assert "incoming.registration_pub_issue" in sql
    assert "application_number >= 'A100'" in sql
    assert "application_number < 'A200'" in sql


def test_registration_resume_skips_successful_range() -> None:
    package = uuid.uuid4()
    store = _Store()
    first_client = _Client(query_results=[[("A300",)], []], fail_command=2)
    first = NativeRegistrationPublicationEventExecutor(
        client=first_client,
    ...
    with pytest.raises(RuntimeError, match=r"task=2/2.*synthetic registration-publication failure"):
        first.execute()

    retry_client = _Client(query_results=[[("A300",)], []])
    retry = NativeRegistrationPublicationEventExecutor(
        client=retry_client,
        package_uuid=package,
        package_kind="MONTHLY_PATCH",
        source_rank=100,
...
def test_registration_cutover_bypasses_delegate(monkeypatch) -> None:
    package = uuid.uuid4()
    store = _Store()
    delegate = _Delegate()
    execution = _Client(query_results=[[]])
    monkeypatch.setattr(
        native_registration_publication_event,
        "get_package",
        lambda package_id: {"package_kind": "MONTHLY_PATCH"},
    )
    client = NativeRegistrationPublicationEventCutoverClient(
        delegate,
...
def test_old_checkpoint_without_marker_keeps_legacy_registration() -> None:
    package = uuid.uuid4()
    delegate = _Delegate()
    client = NativeRegistrationPublicationEventCutoverClient(
        delegate,
    ...
    assert client.native_registration_publication_event_enabled is False
    client.command(_placeholder(str(package)))
    client.assert_final_publish_complete()
