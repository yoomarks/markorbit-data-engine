...
def test_platformization_contract_keeps_cn_runtime_compatibility() -> None:
    ...
    assert cn_dag["native_node_count"] == 15
    ...
    assert (
        contract["compatibility"][
            "cn_agent_priority_madrid_case_current_case_party_current_close_case_scope_relation_scope_carve_out_case_facts_preliminary_registration_exclusive_mark_name_agent_code_event_native_execution"
        ]
        is True
    )
    assert (
