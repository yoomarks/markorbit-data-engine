...
class _Client:
    ...
    def command(self, sql, *args, **kwargs):
        ...
        if self.fail_command is not None and len(self.commands) == self.fail_command:
            raise RuntimeError("synthetic native mark-name-event failure")
...
def test_mark_name_event_sql_is_delta_only_and_bounded() -> None:
    package = uuid.uuid4()
    sql = mark_name_event_sql(
        package,
    ...
    assert "'MARK_NAME_CHANGED_OBSERVED'" in sql
    assert "CAST(NULL, 'Nullable(Date32)')" in sql
    assert "'mark_name'" in sql
    assert "cur.mark_name_raw, incoming.mark_name_raw" in sql
...
def test_mark_name_event_resume_skips_successful_range() -> None:
    package = uuid.uuid4()
    store = _Store()
    first_client = _Client(query_results=[[("A300",)], []], fail_command=2)
    first = NativeMarkNameEventExecutor(
        client=first_client,
    ...
    with pytest.raises(RuntimeError, match=r"task=2/2.*synthetic native mark-name-event failure"):
        first.execute()

    retry_client = _Client(query_results=[[("A300",)], []])
    retry = NativeMarkNameEventExecutor(
        client=retry_client,
        package_uuid=package,
        package_kind="MONTHLY_PATCH",
        source_rank=100,
...
def test_fresh_mark_name_event_cutover_bypasses_delegate(monkeypatch) -> None:
    package = uuid.uuid4()
    store = _Store()
    delegate = _Delegate()
    execution = _Client(query_results=[[]])
    monkeypatch.setattr(
        native_mark_name_event,
        "get_package",
        lambda package_id: {"package_kind": "MONTHLY_PATCH"},
    )
    client = NativeMarkNameEventCutoverClient(
        delegate,
    ...
    assert client.native_mark_name_event_enabled is True
...
def test_old_checkpoint_without_marker_keeps_legacy_mark_name_event() -> None:
    package = uuid.uuid4()
    delegate = _Delegate()
    client = NativeMarkNameEventCutoverClient(
        delegate,
    ...
    assert client.native_mark_name_event_enabled is False
