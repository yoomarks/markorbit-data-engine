...
from app.cn import native_case_current
from app.cn.native_case_current import (
    NativeCaseCurrentCutoverClient,
    NativeCaseCurrentExecutor,
    case_current_sql,
)
...
class _Client:
    ...
    def command(self, sql, *args, **kwargs):
        ...
        if self.fail_command is not None and len(self.commands) == self.fail_command:
            raise RuntimeError("synthetic native case-current failure")
...
def test_native_case_sql_preserves_current_semantics_and_exact_key_filter() -> None:
    package = uuid.uuid4()
    sql = case_current_sql(
        package,
    ...
    assert "INSERT INTO markorbit_facts.cn_case_current" in sql
    ...
    assert "WHERE application_number IN" in sql
    assert "cur.application_number = '' OR cur.source_rank <= 500" in sql
    assert "lowerUTF8(incoming.mark_name_raw)" in sql
    assert "'MONTHLY_PATCH'" in sql
    assert "toDate32('2026-01-31')" in sql
    assert f"toUUID('{package}')" in sql


def test_native_case_nullable_effective_date_matches_legacy() -> None:
    sql = case_current_sql(
        uuid.uuid4(),
...
def test_native_case_resume_skips_successful_range() -> None:
    package = uuid.uuid4()
    store = _Store()
    first_client = _Client(query_results=[[("A300",)], []], fail_command=2)
    executor = NativeCaseCurrentExecutor(
        client=first_client,
    ...
    with pytest.raises(RuntimeError, match=r"task=2/2.*synthetic native case-current failure"):
        executor.execute()

    retry_client = _Client(query_results=[[("A300",)], []])
    retry = NativeCaseCurrentExecutor(
        client=retry_client,
        package_uuid=package,
        package_kind="MONTHLY_PATCH",
        source_effective_date=date(2026, 1, 31),
...
def test_fresh_case_cutover_bypasses_delegate(monkeypatch) -> None:
    package = uuid.uuid4()
    store = _Store()
    delegate = _Delegate()
    ...
    client = NativeCaseCurrentCutoverClient(
        delegate,
    ...
    assert client.native_case_current_enabled is True
...
def test_old_checkpoint_without_marker_keeps_legacy_case_current() -> None:
    package = uuid.uuid4()
    delegate = _Delegate()
    execution = _Client()
    client = NativeCaseCurrentCutoverClient(
    ...
    assert client.native_case_current_enabled is False
    client.command(_placeholder(str(package)))
