...
class _FakeClickHouseClient:
    ...
    def query(self, sql: str):
        ...
        if sql == CLICKHOUSE_TABLES_SQL:
            ...
            rows = [(table, 1, 1024, 10) for table in CRITICAL_TABLES]
...
def test_healthy_state_passes_without_claiming_full_corpus_acceptance() -> None:
    report = _evaluate()

    assert report["checkpoint_version"] == CHECKPOINT_VERSION
    assert report["status"] == "PASS"
    assert report["read_only"] is True
    assert report["evidence_mode"] == "LIGHTWEIGHT_SERVING_CHECKPOINT"
    assert report["expected_package_success"] is True
    assert report["quiescent"] is True
    assert report["core_tables_ready"] is True
    ...
    assert report["package_reprocessed"] is False
...
def test_package_processing_or_schema_drift_blocks() -> None:
    ...
    assert report["core_tables_ready"] is False
    codes = {reason["code"] for reason in report["reasons"]}
    assert "CN_PACKAGE_PROCESSING" in codes
    assert "GOODS_SCHEMA_MISMATCH" in codes


def test_missing_or_non_success_expected_package_blocks() -> None:
    missing = _evaluate(expected_package=None)
    assert missing["status"] == "BLOCKED"
    assert missing["expected_package_success"] is False
    ...
    assert "EXPECTED_PACKAGE_NOT_SUCCESS" in {
        reason["code"] for reason in failed["reasons"]
    }
...
def test_parts_are_aggregated_by_clickhouse_metadata_without_corpus_scan() -> None:
    report = _parts_report(
        [
            ("cn_case_current", 2, 300, 30),
            ("cn_observed_event", 1, 400, 40),
        ]
    )
...
def test_query_contract_is_metadata_control_only() -> None:
    ...
    assert "UNION ALL" in CLICKHOUSE_ACTIVE_PARTS_SQL.upper()
