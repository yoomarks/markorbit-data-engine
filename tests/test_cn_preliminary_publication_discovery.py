...
def test_request_freezes_primary_key_scope_and_read_budget():
    identity = request().query_identity
    ...
    assert identity["scope"]["application_number"] == {
        "start_inclusive": "A000",
        "end_exclusive": "A999",
    }
    assert identity["scope"]["read_budget"] == {
...
def test_request_rejects_invalid_key_bounds_and_page_size():
    with pytest.raises(DiscoveryContractError, match="non-empty"):
        PreliminaryPublicationDiscoveryRequest(
            application_number_start="",
            application_number_end="A100",
        )

    with pytest.raises(DiscoveryContractError, match="lexically less"):
        PreliminaryPublicationDiscoveryRequest(
            application_number_start="A100",
            application_number_end="A100",
        )
...
def test_sql_is_primary_key_aligned_keyset_only_and_bounded():
    sql = build_page_sql(
        request(),
        position=["A002", "00000000-0000-0000-0000-000000000002"],
    )

    assert "markorbit_facts.cn_case_current FINAL" in sql
    assert "application_number >= 'A000'" in sql
    assert "application_number < 'A999'" in sql
    ...
    assert "ORDER BY application_number ASC, toString(case_id) ASC" in sql
    assert "LIMIT 3" in sql
    assert "OFFSET" not in sql.upper()
    assert "JOIN" not in sql.upper()
    assert "prelim_pub_date >=" not in sql
    assert "prelim_pub_date <" not in sql


def test_execute_page_enforces_clickhouse_read_budget():
    ...
    assert client.calls[0][1] == READ_SETTINGS
    assert READ_SETTINGS["max_rows_to_read"] == MAX_ROWS_TO_READ
    assert READ_SETTINGS["max_bytes_to_read"] == MAX_BYTES_TO_READ
    assert READ_SETTINGS["read_overflow_mode"] == "throw"
    assert result["read_budget"] == READ_SETTINGS
...
def test_candidate_output_carries_exact_row_lineage():
    ...
    assert candidate["prelim_pub_date"] == "2026-07-10"
