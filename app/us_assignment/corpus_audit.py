...
AUDIT_VERSION = "US_ASSIGNMENT_MANIFEST_ACCEPTANCE_V1"
...
def build_manifest_acceptance(manifest_path: Path, raw_root: Path) -> dict[str, Any]:
    ...
    warnings = [
        ...
        if item != "assignment_source_sha_verification_not_requested"
    ]

    if not preflight.get("safe"):
        not_ready.append("assignment_manifest_source_preflight_not_ready")
    ...
    for digest in sorted(set(manifest_by_sha) & set(registry_by_sha)):
        ...
        if str(row.get("partition_value") or "") != str(source["effective_date"]):
            mismatch["effective_date"] = {
                "manifest": source["effective_date"],
    ...
    if missing_registry:
        not_ready.append("assignment_manifest_sources_not_registered")
    if extra_registry:
        hard.append("assignment_registry_contains_sources_outside_manifest")
    if metadata_mismatches:
        hard.append("assignment_manifest_registry_metadata_mismatch")
    ...
    if preflight.get("safe") and incomplete_shas:
        not_ready.append("assignment_manifest_replay_not_complete")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Audit full USPTO Assignment replay against the explicit source manifest"
    )
