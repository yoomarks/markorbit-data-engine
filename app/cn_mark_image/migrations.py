...
CREATE TABLE IF NOT EXISTS visual.asset_derivative (
    ...
    canonical_asset_id uuid NOT NULL
        REFERENCES visual.canonical_asset(canonical_asset_id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS acquisition.cn_mark_image_package (
    ...
    reused_canonical_asset_count bigint NOT NULL DEFAULT 0
        CHECK (reused_canonical_asset_count >= 0),
    unique_raw_asset_count bigint NOT NULL DEFAULT 0 CHECK (unique_raw_asset_count >= 0),
    unique_canonical_asset_count bigint NOT NULL DEFAULT 0
        CHECK (unique_canonical_asset_count >= 0),
...
CREATE TABLE IF NOT EXISTS visual.cn_mark_image_observation (
    package_id uuid NOT NULL
        REFERENCES acquisition.cn_mark_image_package(package_id) ON DELETE RESTRICT,
...
@lru_cache(maxsize=1)
def ensure_cn_mark_image_schema() -> None:
