...
def build_platformization_checkpoint(
    *,
    ...
    """Evaluate M1.7 code and promotion readiness without inventing runtime facts.

    Static CI validates the committed operator-evidence contract. M1.6 remains
    code-ready while current lightweight serving-state evidence is absent. A
    repository VERSION of M1.7 is accepted only when the promotion contract also
    validates a committed current serving-state receipt; no package replay or
    fresh full-corpus semantic acceptance is inferred by this checkpoint.
    """
    ...
    if (
        runtime_boundary.get("evidence_mode")
        != "PRIOR_RUNTIME_OPERATOR_ACCEPTED_PLUS_CURRENT_SERVING_STATE"
    ):
        reasons.append({"code": "RUNTIME_ACCEPTANCE_EVIDENCE_MODE_DRIFT"})
    ...
    if runtime_boundary.get("fresh_full_corpus_validation_claimed") is not False:
        reasons.append({"code": "PLATFORM_OVERCLAIMS_FRESH_FULL_CORPUS_VALIDATION"})
    ...
    if promotion.get("full_corpus_semantic_acceptance_claimed") is not False:
        reasons.append({"code": "PROMOTION_CONTRACT_OVERCLAIMS_FULL_CORPUS_SEMANTICS"})
    ...
    return {
        ...
        "promotion_operator_source_comment_body_sha256": promotion.get(
            "operator_source_comment_body_sha256"
        ),
        "promotion_current_serving_state_present": runtime_present,
        "promotion_current_serving_state_valid": runtime_valid,
        "promotion_current_serving_state_sha256": promotion.get(
