...
PACKAGE_ID = uuid.UUID("00000000-0000-0000-0000-00000000a153")
SOURCE_RANK = 987_654_331
SOURCE_SHA = "153" + "a" * 61
PREFIX = "RPE153-"
...
class _FailSecondEventInsert:
    ...
    def command(self, sql: str, *args: Any, **kwargs: Any) -> Any:
        if "INSERT INTO markorbit_facts.cn_observed_event" in sql:
            self._inserts += 1
            if self._inserts == 2:
                raise RuntimeError("fixture registration-publication interruption")
...
class _CompatibilityDelegate:
    ...
    def command(self, sql: str, *args: Any, **kwargs: Any) -> Any:
        ...
        raise AssertionError("native registration-publication SQL leaked to compatibility delegate")
    ...
    with postgres_conn() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                VALUES (%s, 'CN', 'native-registration-publication-fixture.zip',
                        '/fixture/native-registration-publication.zip', 0, %s,
                        'MONTHLY_PATCH', 'MONTH', 'native-registration-publication',
                        DATE '2026-05-31', %s, 'REGISTERED')
                """,
...
def _seed_current(client: Any) -> None:
    old_package = "00000000-0000-0000-0000-00000000b153"
    rows = (
    ...
    values: list[str] = []
    for index, (suffix, rank, reg_date, issue, hash_char) in enumerate(rows, start=1):
        values.append(
            f"""
            (
                toUUID('00000000-0000-0000-0000-0000000153{index:02d}'),
                '{PREFIX}{suffix}', '{PREFIX}{suffix}', '', 'DIRECT_CN', 'OTHER',
                toDate32('2024-01-{index:02d}'), toDate32('{reg_date}'), '{issue}',
    ...
    client.command(
        ...
        (
            ...
            filing_route, number_family, filing_date, registration_pub_date,
            registration_pub_issue, source_package_kind, source_file,
...
def _seed_stage(client: Any, package: str) -> None:
    ...
    values: list[str] = []
    for index, (suffix, reg_date, issue, hash_char) in enumerate(rows, start=1):
        date_sql = "CAST(NULL, 'Nullable(Date32)')" if reg_date is None else f"toDate32('{reg_date}')"
        values.append(
            f"""
            (
                toUUID('{package}'),
                toUUID('00000000-0000-0000-0000-0000000253{index:02d}'),
                toUUID('00000000-0000-0000-0000-0000000353{index:02d}'),
                '{PREFIX}{suffix}', '{PREFIX}{suffix}', '', 'DIRECT_CN', 'OTHER',
                0, toUUID('00000000-0000-0000-0000-0000000453{index:02d}'),
    ...
    client.command(
        ...
        (
            ...
            registration_pub_date, registration_pub_issue, classes, source_file,
...
def _placeholder(package: str) -> str:
    return f"""
        ...
        SELECT 'REGISTRATION_PUBLICATION_OBSERVED'
...
def main() -> None:
    ...
    try:
        ...
        first = NativeRegistrationPublicationEventCutoverClient(
            first_delegate,
        ...
        try:
            ...
            if "fixture registration-publication interruption" not in str(exc):
                raise
        else:
            raise AssertionError("registration-publication interruption did not fire")
        ...
        if int(partial) != 1:
            raise AssertionError(f"expected one committed registration event before failure, got {partial}")

        resumed_delegate = _CompatibilityDelegate()
        resumed = NativeRegistrationPublicationEventCutoverClient(
            resumed_delegate,
        ...
        if not resumed.native_registration_publication_event_enabled:
            raise AssertionError("persisted registration-publication marker was not reused")
        ...
        if resumed_delegate.commands or resumed_delegate.final_assertions != 1:
            raise AssertionError("registration-publication compatibility boundary was not preserved")
        ...
        expected_hash = sha256(
            (
                f"{PREFIX}A100|REGISTRATION_PUBLICATION_OBSERVED|"
                f"registration_publication|{old_value}|{new_value}|{SOURCE_RANK}"
            ).encode("utf-8")
        ).hexdigest().upper().encode("ascii")
        expected = [(
            f"{PREFIX}A100", "REGISTRATION_PUBLICATION_OBSERVED", "2026-01-11",
            "CASE", "registration_publication", old_value, new_value,
            "OFFICIAL_FACT_OBSERVATION", "NOT_DETERMINED", 1.0,
            "MONTHLY_PATCH", "stage-A100.xml", 10, 11,
            b"f" * 64, SOURCE_RANK, expected_hash,
        )]
        if events != expected:
            raise AssertionError(f"unexpected registration-publication events: {events}")
        ...
        if int(converged) != 1:
            raise AssertionError(f"registration event hash replay did not converge: {converged}")
        ...
        print(
            "native REGISTRATION_PUBLICATION_EVENT fixture passed: "
