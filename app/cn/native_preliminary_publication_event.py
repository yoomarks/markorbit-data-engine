...
NATIVE_PRELIMINARY_PUBLICATION_EVENT_VERSION = "CN_NATIVE_PRELIMINARY_PUBLICATION_EVENT_V1"
NATIVE_PRELIMINARY_PUBLICATION_EVENT_CUTOVER_VERSION = (
    "CN_NATIVE_PRELIMINARY_PUBLICATION_EVENT_CUTOVER_V1"
)
NATIVE_PRELIMINARY_PUBLICATION_EVENT_STAGE = "cn_stage_case_publish"
NATIVE_PRELIMINARY_PUBLICATION_EVENT_CUTOVER_STAGE = (
    "__native_preliminary_publication_event_cutover_v1__"
)
NATIVE_PRELIMINARY_PUBLICATION_EVENT_TARGET_ROWS = 25_000
_EVENT_INSERT = "INSERT INTO markorbit_facts.cn_observed_event"
_EVENT_MARKER = "PRELIMINARY_PUBLICATION_OBSERVED"
_EVENT_OPERATION_HASH = sha256(
    (
        f"{NATIVE_PRELIMINARY_PUBLICATION_EVENT_VERSION}|"
        "PRELIMINARY_PUBLICATION_EVENT|SEMANTIC_SQL_V1"
    ).encode("utf-8")
).hexdigest()
_EVENT_CUTOVER_HASH = sha256(
    NATIVE_PRELIMINARY_PUBLICATION_EVENT_CUTOVER_VERSION.encode("utf-8")
).hexdigest()
...
def plan_preliminary_publication_event_ranges(
    package_uuid: uuid.UUID | str,
    *,
    client: Any,
    target_rows: int = NATIVE_PRELIMINARY_PUBLICATION_EVENT_TARGET_ROWS,
    ...
    while True:
        ...
        rows = client.query(
            ...
            FROM markorbit_facts.{NATIVE_PRELIMINARY_PUBLICATION_EVENT_STAGE}
...
def preliminary_publication_event_sql(
    package_uuid: uuid.UUID | str,
    *,
    package_kind: str,
    source_rank: int,
    ...
    old_value = (
        "toJSONString(map('date', ifNull(toString(cur.prelim_pub_date), ''), "
        "'issue', cur.prelim_pub_issue))"
    )
    new_value = (
        "toJSONString(map('date', ifNull(toString(incoming.prelim_pub_date), ''), "
        "'issue', incoming.prelim_pub_issue))"
    )
    return f"""
        INSERT INTO markorbit_facts.cn_observed_event
        SELECT
            generateUUIDv4(), incoming.case_id, incoming.application_number,
            'PRELIMINARY_PUBLICATION_OBSERVED', incoming.prelim_pub_date, now64(3), 'CASE',
            CAST(NULL, 'Nullable(UInt8)'), 'preliminary_publication',
            ...
            hex(SHA256(concat(
                incoming.application_number, '|', 'PRELIMINARY_PUBLICATION_OBSERVED', '|',
                'preliminary_publication', '|', {old_value}, '|', {new_value}, '|',
        ...
        WHERE cur.source_rank < {int(source_rank)}
          AND incoming.prelim_pub_date IS NOT NULL
          AND concat(ifNull(toString(cur.prelim_pub_date), ''), '|', cur.prelim_pub_issue)
              != concat(ifNull(toString(incoming.prelim_pub_date), ''), '|', incoming.prelim_pub_issue)
    """


@dataclass(frozen=True)
class NativePreliminaryPublicationEventExecutionResult:
    range_count: int
    executed: int
    skipped: int


class NativePreliminaryPublicationEventExecutor:
    def __init__(
        self,
        *,
        client: Any,
        ...
        self._result: NativePreliminaryPublicationEventExecutionResult | None = None

    def execute(self) -> NativePreliminaryPublicationEventExecutionResult:
        if self._result is not None:
            raise RuntimeError("native PRELIMINARY_PUBLICATION_EVENT emitted more than once")
        try:
            ranges = plan_preliminary_publication_event_ranges(
                self.package_id,
                client=self.client,
            ...
            raise RuntimeError(
                f"native_publish_subphase=PRELIMINARY_PUBLICATION_EVENT_PLAN failed: {exc}"
            ) from exc
        ...
        for index, (lower, upper) in enumerate(ranges, start=1):
            task_key = self.subtask_store.task_key(
                ...
                stage_table=NATIVE_PRELIMINARY_PUBLICATION_EVENT_STAGE,
                lower=lower,
                upper=upper,
            )
            ...
            self.subtask_store.mark_running(
                task_key=task_key,
                task_group="PRELIMINARY_PUBLICATION_EVENT",
                task_index=index,
                task_total=total,
                stage_table=NATIVE_PRELIMINARY_PUBLICATION_EVENT_STAGE,
                lower=lower,
            ...
            try:
                ...
                raise RuntimeError(
                    "native_publish_subphase=PRELIMINARY_PUBLICATION_EVENT "
        ...
        self._result = NativePreliminaryPublicationEventExecutionResult(total, executed, skipped)
        return self._result

    def assert_complete(self) -> dict[str, int]:
        if self._result is None:
            raise RuntimeError("native PRELIMINARY_PUBLICATION_EVENT was enabled but never observed")
        return {
...
class NativePreliminaryPublicationEventCutoverClient:
    """Versioned native cutover for preliminary-publication delta events."""
    ...
    def __init__(
        ...
        *,
        ...
        if self._target_rows < 1:
            raise ValueError("target_rows must be positive")
        self._executor: NativePreliminaryPublicationEventExecutor | None = None
    ...
    @property
    def native_preliminary_publication_event_enabled(self) -> bool:
    ...
    def command(self, sql: str, *args: Any, **kwargs: Any) -> Any:
        ...
        if node is None or node.task_id != "PRELIMINARY_PUBLICATION_EVENT":
            resolved = node.task_id if node is not None else "NONE"
            raise RuntimeError(
                "native preliminary-publication cutover received unexpected sequencing shape: "
                f"expected=PRELIMINARY_PUBLICATION_EVENT, resolved={resolved}"
            )
        if self._executor is not None:
            raise RuntimeError("native PRELIMINARY_PUBLICATION_EVENT placeholder emitted twice")
        if self._package_kind is None:
            self._package_kind = str(get_package(self._package_id)["package_kind"])
        self._executor = NativePreliminaryPublicationEventExecutor(
    ...
    def assert_preliminary_publication_event_complete(self) -> None:
    ...
    def assert_final_publish_complete(self) -> dict[str, int]:
        self.assert_preliminary_publication_event_complete()
    ...
    def _initialize_cutover(self, allow_new_cutover: bool) -> bool:
        marker_key = self._subtask_store.task_key(
            ...
            stage_table=NATIVE_PRELIMINARY_PUBLICATION_EVENT_CUTOVER_STAGE,
            lower=None,
            upper=None,
        )
    ...
    def _write_cutover_marker(self, marker_key: str) -> None:
        self._subtask_store.mark_running(
            task_key=marker_key,
            task_group="NATIVE_PRELIMINARY_PUBLICATION_EVENT_CUTOVER",
            task_index=1,
            task_total=1,
            stage_table=NATIVE_PRELIMINARY_PUBLICATION_EVENT_CUTOVER_STAGE,
            lower=None,
...
def native_preliminary_publication_event_contract() -> dict[str, Any]:
    return {
        "version": NATIVE_PRELIMINARY_PUBLICATION_EVENT_VERSION,
        "cutover_version": NATIVE_PRELIMINARY_PUBLICATION_EVENT_CUTOVER_VERSION,
        "native_node": "PRELIMINARY_PUBLICATION_EVENT",
        "partition": "WHOLE_APPLICATION_HALF_OPEN_RANGE",
        "target_rows": NATIVE_PRELIMINARY_PUBLICATION_EVENT_TARGET_ROWS,
        "durable_resume": True,
        ...
        "required_incoming_field": "PRELIM_PUB_DATE_NOT_NULL",
        "change_semantics": "DATE_OR_ISSUE_CHANGED",
        "baseline_policy": "NO_FIRST_OBSERVATION_EVENT",
        "preexisting_checkpoint_policy": "KEEP_LEGACY_PRELIMINARY_PUBLICATION_EVENT_UNLESS_MARKER_PRESENT",
    }
