...
NATIVE_CASE_FACTS_EVENT_VERSION = "CN_NATIVE_CASE_FACTS_EVENT_V1"
NATIVE_CASE_FACTS_EVENT_CUTOVER_VERSION = "CN_NATIVE_CASE_FACTS_EVENT_CUTOVER_V1"
NATIVE_CASE_FACTS_EVENT_STAGE = "cn_stage_case_publish"
NATIVE_CASE_FACTS_EVENT_CUTOVER_STAGE = "__native_case_facts_event_cutover_v1__"
NATIVE_CASE_FACTS_EVENT_TARGET_ROWS = 25_000
_EVENT_INSERT = "INSERT INTO markorbit_facts.cn_observed_event"
_EVENT_MARKER = "CASE_FACTS_CHANGED_OBSERVED"
_EVENT_OPERATION_HASH = sha256(
    f"{NATIVE_CASE_FACTS_EVENT_VERSION}|CASE_FACTS_EVENT|SEMANTIC_SQL_V1".encode("utf-8")
).hexdigest()
_EVENT_CUTOVER_HASH = sha256(
    NATIVE_CASE_FACTS_EVENT_CUTOVER_VERSION.encode("utf-8")
).hexdigest()
...
def plan_case_facts_event_ranges(
    package_uuid: uuid.UUID | str,
    *,
    client: Any,
    target_rows: int = NATIVE_CASE_FACTS_EVENT_TARGET_ROWS,
    ...
    while True:
        ...
        rows = client.query(
            ...
            FROM markorbit_facts.{NATIVE_CASE_FACTS_EVENT_STAGE}
    ...
    *,
    ...
    return f"""
        ...
        SELECT
            ...
            'CASE_FACTS_CHANGED_OBSERVED', incoming.filing_date, now64(3), 'CASE',
            CAST(NULL, 'Nullable(UInt8)'), 'record_hash',
            cur.record_hash, incoming.record_hash, 'OFFICIAL_FACT_OBSERVATION',
            ...
            hex(SHA256(concat(
                incoming.application_number, '|', 'CASE_FACTS_CHANGED_OBSERVED', '|',
                'record_hash', '|', cur.record_hash, '|', incoming.record_hash, '|',
...
class NativeCaseFactsEventExecutionResult:
    range_count: int
    executed: int
    skipped: int


class NativeCaseFactsEventExecutor:
    def __init__(
        self,
        *,
        client: Any,
        ...
        self._result: NativeCaseFactsEventExecutionResult | None = None

    def execute(self) -> NativeCaseFactsEventExecutionResult:
        if self._result is not None:
            raise RuntimeError("native CASE_FACTS_EVENT emitted more than once")
        try:
            ranges = plan_case_facts_event_ranges(
                self.package_id,
                client=self.client,
                target_rows=self.target_rows,
            )
        except Exception as exc:
            raise RuntimeError(f"native_publish_subphase=CASE_FACTS_EVENT_PLAN failed: {exc}") from exc
        ...
        for index, (lower, upper) in enumerate(ranges, start=1):
            task_key = self.subtask_store.task_key(
                ...
                stage_table=NATIVE_CASE_FACTS_EVENT_STAGE,
                lower=lower,
                upper=upper,
            )
            ...
            try:
                ...
                raise RuntimeError(
                    "native_publish_subphase=CASE_FACTS_EVENT "
        ...
        self._result = NativeCaseFactsEventExecutionResult(total, executed, skipped)
        return self._result

    def assert_complete(self) -> dict[str, int]:
        if self._result is None:
            raise RuntimeError("native CASE_FACTS_EVENT was enabled but never observed")
        return {
...
class NativeCaseFactsEventCutoverClient:
    """Versioned native cutover for CASE_FACTS_CHANGED_OBSERVED delta events."""
    ...
    def __init__(
        ...
        *,
        ...
        if self._target_rows < 1:
            raise ValueError("target_rows must be positive")
        self._executor: NativeCaseFactsEventExecutor | None = None
    ...
    @property
    def native_case_facts_event_enabled(self) -> bool:
    ...
    def command(self, sql: str, *args: Any, **kwargs: Any) -> Any:
        ...
        if node is None or node.task_id != "CASE_FACTS_EVENT":
            resolved = node.task_id if node is not None else "NONE"
            raise RuntimeError(
                "native case-facts-event cutover received unexpected sequencing shape: "
                f"expected=CASE_FACTS_EVENT, resolved={resolved}"
            )
        if self._executor is not None:
            raise RuntimeError("native CASE_FACTS_EVENT placeholder emitted twice")
        if self._package_kind is None:
            self._package_kind = str(get_package(self._package_id)["package_kind"])
        self._executor = NativeCaseFactsEventExecutor(
    ...
    def assert_case_facts_event_complete(self) -> None:
    ...
    def assert_final_publish_complete(self) -> dict[str, int]:
        self.assert_case_facts_event_complete()
    ...
    def _initialize_cutover(self, allow_new_cutover: bool) -> bool:
        marker_key = self._subtask_store.task_key(
            ...
            stage_table=NATIVE_CASE_FACTS_EVENT_CUTOVER_STAGE,
            lower=None,
            upper=None,
        )
    ...
    def _write_cutover_marker(self, marker_key: str) -> None:
        self._subtask_store.mark_running(
            task_key=marker_key,
            task_group="NATIVE_CASE_FACTS_EVENT_CUTOVER",
            task_index=1,
            task_total=1,
            stage_table=NATIVE_CASE_FACTS_EVENT_CUTOVER_STAGE,
            lower=None,
...
def native_case_facts_event_contract() -> dict[str, Any]:
    return {
        "version": NATIVE_CASE_FACTS_EVENT_VERSION,
        "cutover_version": NATIVE_CASE_FACTS_EVENT_CUTOVER_VERSION,
        "native_node": "CASE_FACTS_EVENT",
        "partition": "WHOLE_APPLICATION_HALF_OPEN_RANGE",
        "target_rows": NATIVE_CASE_FACTS_EVENT_TARGET_ROWS,
        "durable_resume": True,
        ...
        "preexisting_checkpoint_policy": "KEEP_LEGACY_CASE_FACTS_EVENT_UNLESS_MARKER_PRESENT",
    }
