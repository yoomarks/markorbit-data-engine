...
from app.cn.native_party_observed_event import (
    NativePartyObservedEventCutoverClient,
...
PACKAGE_ID = uuid.UUID("00000000-0000-0000-0000-00000000a159")
SOURCE_RANK = 987_654_337
SOURCE_SHA = "159" + "b" * 61
PREFIX = "POE159-"
...
class _FailSecondEventInsert:
    ...
    def command(self, sql: str, *args: Any, **kwargs: Any) -> Any:
        if "INSERT INTO markorbit_facts.cn_observed_event" in sql:
            self._inserts += 1
            if self._inserts == 2:
                raise RuntimeError("fixture native party-observed interruption")
...
class _CompatibilityDelegate:
    ...
    def command(self, sql: str, *args: Any, **kwargs: Any) -> Any:
        ...
        raise AssertionError("native PARTY_OBSERVED_EVENT SQL leaked to compatibility delegate")
    ...
    with postgres_conn() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                VALUES (%s, 'CN', 'native-party-observed-event-fixture.zip',
                        '/fixture/native-party-observed-event.zip', 0, %s,
                        'MONTHLY_PATCH', 'MONTH', 'native-party-observed-event',
...
def _seed_current(client: Any) -> None:
    old_package = "00000000-0000-0000-0000-00000000b159"
    rows = (
        ("A200", "OWNER", "2", SOURCE_RANK - 1, 1, "B"),
        ("A300", "OWNER", "3", SOURCE_RANK - 1, 1, "X"),
        ("A400", "AGENT", "4", SOURCE_RANK - 1, 0, "D"),
        ("A500", "OWNER", "5", SOURCE_RANK, 1, "Y"),
        ("A600", "OWNER", "6", SOURCE_RANK + 1, 1, "Z"),
    )
    values: list[str] = []
    for index, (suffix, role, key_char, rank, is_current, record_char) in enumerate(rows, start=2):
        relation_id = f"00000000-0000-0000-0000-0000000159{index:02d}"
        case_id = f"00000000-0000-0000-0000-0000000259{index:02d}"
        mention_id = f"00000000-0000-0000-0000-0000000359{index:02d}"
        valid_to = "CAST(NULL, 'Nullable(Date32)')" if is_current else "toDate32('2026-01-31')"
        ...
        values.append(
            ...
            (
                ...
                CAST(NULL, 'Nullable(UUID)'), '', 'Prior {role}', lowerUTF8('Prior {role}'),
                'Prior Address', 'prior address', 'CN', 'BJ', 'Beijing', [9], 0.90,
                toDate32('2025-01-01'), {valid_to}, {is_current}, '{status}',
                'CASE_ROLE_REPLACE', 'MONTHLY_PATCH', toDate32('2026-01-31'),
                'old-{suffix}.xml', 1, 2, '{record_char.lower() * 64}',
...
def _seed_stage(client: Any, package: str) -> None:
    rows = (
        ("A100", "OWNER", "1", "First Owner", "Address 100", None, "A", "q"),
        ("A200", "OWNER", "2", "Same Owner", "Address 200", None, "B", "r"),
        (
            "A300", "OWNER", "3", "Changed Owner", "Address 300",
            "00000000-0000-0000-0000-000000075903", "C", "s",
        ),
        ("A400", "AGENT", "4", "Reactivated Agent", "", None, "D", "t"),
        ("A500", "OWNER", "5", "Equal Rank Owner", "Address 500", None, "E", "u"),
        ("A600", "OWNER", "6", "Newer Rank Owner", "Address 600", None, "F", "v"),
    )
    values: list[str] = []
    for index, (suffix, role, key_char, name, address, entity_id, record_char, source_char) in enumerate(
        rows, start=1
    ):
        relation_id = f"00000000-0000-0000-0000-0000000459{index:02d}"
        case_id = f"00000000-0000-0000-0000-0000000559{index:02d}"
        mention_id = f"00000000-0000-0000-0000-0000000659{index:02d}"
        entity_expr = (
            f"toNullable(toUUID('{entity_id}'))"
            if entity_id
            else "CAST(NULL, 'Nullable(UUID)')"
        )
        values.append(
            f"""
            (
                ...
                toUUID('{mention_id}'), {entity_expr}, '',
                '{name}', lowerUTF8('{name}'), '{address}', lowerUTF8('{address}'),
                'CN', 'BJ', 'Beijing', [9], 0.95, 'stage-{suffix.lower()}.xml',
                {index * 100}, {index * 100 + 1}, '{source_char * 64}', '{record_char * 64}'
            )
            """
        )
    client.command(
...
def _event_placeholder(package: str) -> str:
    return f"""
        ...
        SELECT concat(incoming.role, '_RELATION_OBSERVED')
...
def _expected_event(
    ...
    relation_key = key_char * 64
    event_hash = sha256(
        f"{PREFIX}{suffix}|{role}|OBSERVED|{relation_key}|{SOURCE_RANK}".encode("utf-8")
    ).hexdigest().upper().encode("ascii")
    new_value = (
        '{"name":"' + name + '","address":"' + address +
        '","relation_key":"' + relation_key + '","entity_id":"' + entity_id + '"}'
    )
    return (
        f"{PREFIX}{suffix}", f"{role}_RELATION_OBSERVED", EFFECTIVE_DATE,
        "PARTY", None, role.lower(), "", new_value,
        "OFFICIAL_FACT_OBSERVATION", "NOT_DETERMINED", "1",
        "MONTHLY_PATCH", f"stage-{suffix.lower()}.xml", line, line + 1,
        source_char.encode("ascii") * 64, SOURCE_RANK, event_hash,
    )
...
def main() -> None:
    ...
    try:
        ...
        first_native = NativePartyObservedEventCutoverClient(
            first_delegate,
        ...
        try:
            ...
            if "fixture native party-observed interruption" not in str(exc):
                raise
        else:
            raise AssertionError("native PARTY_OBSERVED_EVENT interruption did not fire")
        ...
        if int(partial) != 1:
            raise AssertionError(f"expected one committed observed event before failure, got {partial}")

        resumed_delegate = _CompatibilityDelegate()
        resumed_native = NativePartyObservedEventCutoverClient(
            resumed_delegate,
        ...
        if not resumed_native.native_party_observed_event_enabled:
            raise AssertionError("persisted PARTY_OBSERVED_EVENT marker was not reused")
        ...
        if resumed_delegate.commands or resumed_delegate.final_assertions != 1:
            raise AssertionError("PARTY_OBSERVED_EVENT compatibility boundary was not preserved")
        ...
        expected = [
            _expected_event("A100", "OWNER", "1", "First Owner", "Address 100", "", "q", 100),
            _expected_event(
                "A300", "OWNER", "3", "Changed Owner", "Address 300",
                "00000000-0000-0000-0000-000000075903", "s", 300,
            ),
            _expected_event("A400", "AGENT", "4", "Reactivated Agent", "", "", "t", 400),
        ]
        if events != expected:
            raise AssertionError(f"unexpected native PARTY_OBSERVED_EVENT rows: {events}")
        ...
        if int(converged) != 1:
            raise AssertionError(f"party observed event_hash replay did not converge: {converged}")
        ...
        print(
            "native PARTY_OBSERVED_EVENT fixture passed: "
