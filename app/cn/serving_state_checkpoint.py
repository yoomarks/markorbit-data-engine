...
CHECKPOINT_VERSION = "CN_LIGHTWEIGHT_SERVING_STATE_V1"
...
CLICKHOUSE_ACTIVE_PARTS_SQL = f"""
    SELECT table, bytes_on_disk, rows
    FROM system.parts
    WHERE database = 'markorbit_facts'
      AND active
      AND table IN ({_TABLE_LIST_SQL})
    ORDER BY table, name
"""
...
def _aggregate_parts(rows: list[tuple[Any, Any, Any]]) -> dict[str, dict[str, int]]:
    result = {
        table: {"active_parts": 0, "bytes_on_disk": 0, "rows_from_parts": 0}
        for table in CRITICAL_TABLES
    }
    for table, bytes_on_disk, rows_count in rows:
        table_name = str(table)
        if table_name not in result:
            continue
        item = result[table_name]
        item["active_parts"] += 1
        item["bytes_on_disk"] += int(bytes_on_disk or 0)
        item["rows_from_parts"] += int(rows_count or 0)
    return result
...
def evaluate_serving_state(
    *,
    ...
    elif str(expected_package.get("status") or "") != "SUCCESS":
        reasons.append(
            _reason(
    ...
    if goods_schema_error:
        reasons.append(_reason("GOODS_SCHEMA_MISMATCH", goods_schema_error, "BLOCKED"))
...
def build_serving_state_checkpoint(
    ...
    *,
    ...
    parts = _aggregate_parts(client.query(CLICKHOUSE_ACTIVE_PARTS_SQL).result_rows)
