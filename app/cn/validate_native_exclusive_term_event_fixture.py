...
PACKAGE_ID = uuid.UUID("00000000-0000-0000-0000-00000000a154")
SOURCE_RANK = 987_654_332
SOURCE_SHA = "154" + "a" * 61
PREFIX = "ETE154-"
...
class _FailSecondEventInsert:
    ...
    def command(self, sql: str, *args: Any, **kwargs: Any) -> Any:
        if "INSERT INTO markorbit_facts.cn_observed_event" in sql:
            self._inserts += 1
            if self._inserts == 2:
                raise RuntimeError("fixture exclusive-term interruption")
...
class _CompatibilityDelegate:
    ...
    def command(self, sql: str, *args: Any, **kwargs: Any) -> Any:
        ...
        raise AssertionError("native exclusive-term SQL leaked to compatibility delegate")
    ...
    with postgres_conn() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                VALUES (%s, 'CN', 'native-exclusive-term-fixture.zip',
                        '/fixture/native-exclusive-term.zip', 0, %s,
                        'MONTHLY_PATCH', 'MONTH', 'native-exclusive-term',
                        DATE '2026-06-30', %s, 'REGISTERED')
                """,
...
def _seed_current(client: Any) -> None:
    old_package = "00000000-0000-0000-0000-00000000b154"
    rows = (
        ("A100", SOURCE_RANK - 1, "2020-01-01", "2030-01-01", "A"),
        ("A200", SOURCE_RANK - 1, "2020-02-02", "2030-02-02", "B"),
        ("A300", SOURCE_RANK - 1, "2020-03-03", "2030-03-03", "C"),
        ("A400", SOURCE_RANK, "2020-04-04", "2030-04-04", "D"),
        ("A600", SOURCE_RANK - 1, "2020-06-06", "2030-06-06", "E"),
        ("A700", SOURCE_RANK + 1, "2020-07-07", "2030-07-07", "F"),
    )
    values: list[str] = []
    for index, (suffix, rank, valid_from, valid_until, hash_char) in enumerate(rows, start=1):
        values.append(
            f"""
            (
                toUUID('00000000-0000-0000-0000-0000000154{index:02d}'),
                '{PREFIX}{suffix}', '{PREFIX}{suffix}', '', 'DIRECT_CN', 'OTHER',
                toDate32('2019-01-{index:02d}'), toDate32('{valid_from}'),
                toDate32('{valid_until}'), 'MONTHLY_PATCH', 'old-{suffix}.xml', 1, 2,
    ...
    client.command(
        ...
        (
            ...
            filing_route, number_family, filing_date, valid_from, valid_until,
...
def _seed_stage(client: Any, package: str) -> None:
    rows = (
        ("A100", "2020-01-01", "2035-01-01", "2020-01-01 至 2035-01-01", "G"),
        ("A200", "2021-02-02", "2029-02-02", "2021-02-02 至 2029-02-02", "H"),
        ("A300", "2020-03-03", "2030-03-03", "same period", "I"),
        ("A400", "2021-04-04", "2031-04-04", "same-rank change", "J"),
        ("A500", "2021-05-05", "2031-05-05", "no prior current", "K"),
        ("A600", None, None, "raw only", "L"),
        ("A700", "2021-07-07", "2031-07-07", "newer current", "M"),
    )
    values: list[str] = []
    for index, (suffix, start_date, end_date, raw, hash_char) in enumerate(rows, start=1):
        start_sql = "CAST(NULL, 'Nullable(Date32)')" if start_date is None else f"toDate32('{start_date}')"
        end_sql = "CAST(NULL, 'Nullable(Date32)')" if end_date is None else f"toDate32('{end_date}')"
        values.append(
            f"""
            (
                toUUID('{package}'),
                toUUID('00000000-0000-0000-0000-0000000254{index:02d}'),
                toUUID('00000000-0000-0000-0000-0000000354{index:02d}'),
                '{PREFIX}{suffix}', '{PREFIX}{suffix}', '', 'DIRECT_CN', 'OTHER',
                0, toUUID('00000000-0000-0000-0000-0000000454{index:02d}'),
                'Mark {suffix}', toDate32('2019-02-{index:02d}'),
                {start_sql}, {end_sql}, '{raw}', [9], 'stage-{suffix}.xml',
    ...
    client.command(
        ...
        (
            ...
            exclusive_start_date, exclusive_end_date, exclusive_period, classes,
...
def main() -> None:
    ...
    try:
        ...
        first = NativeExclusiveTermEventCutoverClient(
            first_delegate,
        ...
        try:
            ...
            if "fixture exclusive-term interruption" not in str(exc):
                raise
        else:
            raise AssertionError("exclusive-term interruption did not fire")
        ...
        if int(partial) != 1:
            raise AssertionError(f"expected one committed exclusive-term event before failure, got {partial}")

        resumed_delegate = _CompatibilityDelegate()
        resumed = NativeExclusiveTermEventCutoverClient(
            resumed_delegate,
        ...
        if not resumed.native_exclusive_term_event_enabled:
            raise AssertionError("persisted exclusive-term marker was not reused")
        result = resumed.command(_placeholder(package))
        resumed.assert_final_publish_complete()
        if result.range_count != 7 or result.skipped != 1 or result.executed != 6:
            raise AssertionError(result)
        if resumed_delegate.commands or resumed_delegate.final_assertions != 1:
            raise AssertionError("exclusive-term compatibility boundary was not preserved")
        ...
        old_a100 = '{"from":"2020-01-01","until":"2030-01-01"}'
        new_a100 = '{"from":"2020-01-01","until":"2035-01-01","raw":"2020-01-01 至 2035-01-01"}'
        hash_a100 = sha256(
            (
                f"{PREFIX}A100|TERM_EXTENDED_OBSERVED|exclusive_term|"
                f"{old_a100}|{new_a100}|{SOURCE_RANK}"
            ).encode("utf-8")
        ).hexdigest().upper().encode("ascii")
        old_a200 = '{"from":"2020-02-02","until":"2030-02-02"}'
        new_a200 = '{"from":"2021-02-02","until":"2029-02-02","raw":"2021-02-02 至 2029-02-02"}'
        hash_a200 = sha256(
            (
                f"{PREFIX}A200|EXCLUSIVE_TERM_OBSERVED|exclusive_term|"
                f"{old_a200}|{new_a200}|{SOURCE_RANK}"
            ).encode("utf-8")
        ).hexdigest().upper().encode("ascii")
        expected = [
            (
                f"{PREFIX}A100", "TERM_EXTENDED_OBSERVED", "2035-01-01",
                "CASE", "exclusive_term", old_a100, new_a100,
                "OFFICIAL_FACT_OBSERVATION", "NOT_DETERMINED", 1.0,
                "MONTHLY_PATCH", "stage-A100.xml", 10, 11,
                b"g" * 64, SOURCE_RANK, hash_a100,
            ),
            (
                f"{PREFIX}A200", "EXCLUSIVE_TERM_OBSERVED", "2029-02-02",
                "CASE", "exclusive_term", old_a200, new_a200,
                "OFFICIAL_FACT_OBSERVATION", "NOT_DETERMINED", 1.0,
                "MONTHLY_PATCH", "stage-A200.xml", 20, 21,
                b"h" * 64, SOURCE_RANK, hash_a200,
            ),
        ]
        if events != expected:
            raise AssertionError(f"unexpected exclusive-term events: {events}")
        ...
        if int(converged) != 1:
            raise AssertionError(f"exclusive-term event hash replay did not converge: {converged}")
        ...
        print(
            "native EXCLUSIVE_TERM_EVENT fixture passed: "
