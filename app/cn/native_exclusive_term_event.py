...
NATIVE_EXCLUSIVE_TERM_EVENT_VERSION = "CN_NATIVE_EXCLUSIVE_TERM_EVENT_V1"
NATIVE_EXCLUSIVE_TERM_EVENT_CUTOVER_VERSION = "CN_NATIVE_EXCLUSIVE_TERM_EVENT_CUTOVER_V1"
NATIVE_EXCLUSIVE_TERM_EVENT_STAGE = "cn_stage_case_publish"
NATIVE_EXCLUSIVE_TERM_EVENT_CUTOVER_STAGE = "__native_exclusive_term_event_cutover_v1__"
NATIVE_EXCLUSIVE_TERM_EVENT_TARGET_ROWS = 25_000
_EVENT_INSERT = "INSERT INTO markorbit_facts.cn_observed_event"
_EVENT_MARKER = "TERM_EXTENDED_OBSERVED"
_EVENT_OPERATION_HASH = sha256(
    f"{NATIVE_EXCLUSIVE_TERM_EVENT_VERSION}|EXCLUSIVE_TERM_EVENT|SEMANTIC_SQL_V1".encode(
        "utf-8"
    )
).hexdigest()
_EVENT_CUTOVER_HASH = sha256(
    NATIVE_EXCLUSIVE_TERM_EVENT_CUTOVER_VERSION.encode("utf-8")
).hexdigest()
...
def plan_exclusive_term_event_ranges(
    package_uuid: uuid.UUID | str,
    *,
    client: Any,
    target_rows: int = NATIVE_EXCLUSIVE_TERM_EVENT_TARGET_ROWS,
    ...
    while True:
        ...
        rows = client.query(
            ...
            FROM markorbit_facts.{NATIVE_EXCLUSIVE_TERM_EVENT_STAGE}
...
def exclusive_term_event_sql(
    ...
    *,
    ...
    event_type = (
        "if(cur.valid_until IS NOT NULL AND incoming.exclusive_end_date > cur.valid_until, "
        "'TERM_EXTENDED_OBSERVED', 'EXCLUSIVE_TERM_OBSERVED')"
    )
    old_value = (
        "toJSONString(map('from', ifNull(toString(cur.valid_from), ''), "
        "'until', ifNull(toString(cur.valid_until), '')))"
    )
    new_value = (
        "toJSONString(map('from', ifNull(toString(incoming.exclusive_start_date), ''), "
        "'until', ifNull(toString(incoming.exclusive_end_date), ''), "
        "'raw', incoming.exclusive_period))"
    )
    return f"""
        INSERT INTO markorbit_facts.cn_observed_event
        SELECT
            generateUUIDv4(), incoming.case_id, incoming.application_number,
            {event_type}, incoming.exclusive_end_date, now64(3), 'CASE',
            CAST(NULL, 'Nullable(UInt8)'), 'exclusive_term',
            ...
            hex(SHA256(concat(
                incoming.application_number, '|', {event_type}, '|', 'exclusive_term', '|',
                {old_value}, '|', {new_value}, '|', toString({int(source_rank)})
            )))
        FROM
        (
            SELECT *
        ...
        WHERE cur.source_rank < {int(source_rank)}
          AND (incoming.exclusive_start_date IS NOT NULL OR incoming.exclusive_end_date IS NOT NULL)
          AND concat(ifNull(toString(cur.valid_from), ''), '|', ifNull(toString(cur.valid_until), ''))
              != concat(ifNull(toString(incoming.exclusive_start_date), ''), '|',
                        ifNull(toString(incoming.exclusive_end_date), ''))
    """


@dataclass(frozen=True)
class NativeExclusiveTermEventExecutionResult:
    range_count: int
    executed: int
    skipped: int


class NativeExclusiveTermEventExecutor:
    def __init__(
        self,
        *,
        client: Any,
        ...
        self._result: NativeExclusiveTermEventExecutionResult | None = None

    def execute(self) -> NativeExclusiveTermEventExecutionResult:
        if self._result is not None:
            raise RuntimeError("native EXCLUSIVE_TERM_EVENT emitted more than once")
        try:
            ranges = plan_exclusive_term_event_ranges(
                self.package_id,
                client=self.client,
            ...
            raise RuntimeError(
                f"native_publish_subphase=EXCLUSIVE_TERM_EVENT_PLAN failed: {exc}"
            ) from exc
        ...
        for index, (lower, upper) in enumerate(ranges, start=1):
            task_key = self.subtask_store.task_key(
                ...
                stage_table=NATIVE_EXCLUSIVE_TERM_EVENT_STAGE,
                lower=lower,
                upper=upper,
            )
            ...
            self.subtask_store.mark_running(
                task_key=task_key,
                task_group="EXCLUSIVE_TERM_EVENT",
                task_index=index,
                task_total=total,
                stage_table=NATIVE_EXCLUSIVE_TERM_EVENT_STAGE,
                lower=lower,
            ...
            try:
                ...
                raise RuntimeError(
                    "native_publish_subphase=EXCLUSIVE_TERM_EVENT "
        ...
        self._result = NativeExclusiveTermEventExecutionResult(total, executed, skipped)
        return self._result

    def assert_complete(self) -> dict[str, int]:
        if self._result is None:
            raise RuntimeError("native EXCLUSIVE_TERM_EVENT was enabled but never observed")
        return {
...
class NativeExclusiveTermEventCutoverClient:
    """Versioned native cutover for exclusive-term delta events."""
    ...
    def __init__(
        ...
        *,
        ...
        if self._target_rows < 1:
            raise ValueError("target_rows must be positive")
        self._executor: NativeExclusiveTermEventExecutor | None = None
    ...
    @property
    def native_exclusive_term_event_enabled(self) -> bool:
    ...
    def command(self, sql: str, *args: Any, **kwargs: Any) -> Any:
        ...
        if node is None or node.task_id != "EXCLUSIVE_TERM_EVENT":
            resolved = node.task_id if node is not None else "NONE"
            raise RuntimeError(
                "native exclusive-term cutover received unexpected sequencing shape: "
                f"expected=EXCLUSIVE_TERM_EVENT, resolved={resolved}"
            )
        if self._executor is not None:
            raise RuntimeError("native EXCLUSIVE_TERM_EVENT placeholder emitted twice")
        if self._package_kind is None:
            self._package_kind = str(get_package(self._package_id)["package_kind"])
        self._executor = NativeExclusiveTermEventExecutor(
    ...
    def assert_exclusive_term_event_complete(self) -> None:
    ...
    def assert_final_publish_complete(self) -> dict[str, int]:
        self.assert_exclusive_term_event_complete()
    ...
    def _initialize_cutover(self, allow_new_cutover: bool) -> bool:
        marker_key = self._subtask_store.task_key(
            ...
            stage_table=NATIVE_EXCLUSIVE_TERM_EVENT_CUTOVER_STAGE,
            lower=None,
            upper=None,
        )
    ...
    def _write_cutover_marker(self, marker_key: str) -> None:
        self._subtask_store.mark_running(
            task_key=marker_key,
            task_group="NATIVE_EXCLUSIVE_TERM_EVENT_CUTOVER",
            task_index=1,
            task_total=1,
            stage_table=NATIVE_EXCLUSIVE_TERM_EVENT_CUTOVER_STAGE,
            lower=None,
...
def native_exclusive_term_event_contract() -> dict[str, Any]:
    return {
        "version": NATIVE_EXCLUSIVE_TERM_EVENT_VERSION,
        "cutover_version": NATIVE_EXCLUSIVE_TERM_EVENT_CUTOVER_VERSION,
        "native_node": "EXCLUSIVE_TERM_EVENT",
        "partition": "WHOLE_APPLICATION_HALF_OPEN_RANGE",
        "target_rows": NATIVE_EXCLUSIVE_TERM_EVENT_TARGET_ROWS,
        "durable_resume": True,
        ...
        "event_type_semantics": "TERM_EXTENDED_IF_END_DATE_EXTENDS_PRIOR_VALID_UNTIL_ELSE_EXCLUSIVE_TERM",
        "change_semantics": "START_OR_END_PRESENT_AND_PERIOD_CHANGED",
        "baseline_policy": "NO_FIRST_OBSERVATION_EVENT",
        "preexisting_checkpoint_policy": "KEEP_LEGACY_EXCLUSIVE_TERM_EVENT_UNLESS_MARKER_PRESENT",
    }
