...
def _flag(value: str) -> bool:
    # Official TDXF uses T/F for most indicators. Some transforms use Y/N or 1/0.
    return value.strip().upper() in {"T", "Y", "YES", "TRUE", "1"}


def _current_flag(header: ET.Element, current_names: tuple[str, ...], filed: bool) -> bool:
    raw = _text(header, *current_names)
    return _flag(raw) if raw else filed
...
def _parse_case(case_element: ET.Element, source_name: str) -> USCaseRecord:
    ...
    itu_filed = _flag(
        _text(
            header,
            "intent-to-use-in",
            "intent-to-use-application-filed-in",
            "intent-to-use-1b",
        )
    )
    ...
    d44_filed = _flag(
        _text(
            ...
            "foreign-application-filed-in",
            "foreign-application-44d",
        )
    )
    ...
    a66_filed = _flag(
        _text(
            ...
            "international-registration-filed-in",
            "madrid-66a",
        )
    )
    ...
    return USCaseRecord(
        ...
        law_office_code=_text(
            header,
            "law-office-assigned-location-code",
            "law-office-code",
        ),
        standard_character_claimed=_flag(
            _text(header, "standard-characters-claimed-in", "standard-character-claim")
        ),
        use_1a=use_current,
        ...
        international_registration_date=parse_uspto_date(
            _text(intl_source, "international-registration-date")
        ),
        international_publication_date=parse_uspto_date(
            _text(intl_source, "international-publication-date")
        ),
        international_renewal_date=parse_uspto_date(
            _text(intl_source, "international-renewal-date")
        ),
        international_auto_protection_date=parse_uspto_date(
            _text(intl_source, "auto-protection-date")
        ),
        international_death_date=parse_uspto_date(
            _text(intl_source, "international-death-date")
        ),
...
def _parse_owners(
    ...
    for element in _children(case_element, "case-file-owner"):
        ...
        nat_country = (
            _direct_text(nationality, "country", "country-code")
            if nationality is not None
            else ""
        )
        nat_state = (
            _direct_text(nationality, "state", "state-code")
            if nationality is not None
            else ""
        )
        ...
        owners.append(
            USOwnerRecord(
                ...
                nationality_country=nat_country
                or _direct_text(
                    element,
                    "nationality-country",
                    "nationality-country-code",
                ),
                nationality_state=nat_state
                or _direct_text(
                    element,
                    "nationality-state",
...
def _parse_classifications(
    ...
    for element in _children(case_element, "classification"):
        first_use_raw = _direct_text(
            element,
            "first-use-anywhere-date",
            "first-use-anywhere-dt",
            "first-use-date",
        )
        first_commerce_raw = _direct_text(
            element,
        ...
        records.append(
            USClassificationRecord(
                ...
                status_date=parse_uspto_date(
                    _direct_text(element, "status-date", "status-dt")
                ),
        ...
        events.append(
            USEventRecord(
                ...
                event_date=parse_uspto_date(
                    _direct_text(element, "date", "event-date", "event-dt")
                ),
                event_sequence=_integer(
                    _direct_text(element, "number", "event-sequence", "event-seq")
                ),
                event_type_code=_direct_text(
                    element,
                    "type",
                    "event-type-code",
                    "event-type-cd",
                ),
                description_text=_direct_text(
                    element,
                    "description-text",
                    "description",
                    "text",
