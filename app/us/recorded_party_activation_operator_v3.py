...
PLAN_VERSION = "US_RECORDED_PARTY_HISTORY_ACTIVATION_PLAN_V3"
PROGRESS_VERSION = "US_RECORDED_PARTY_HISTORY_ACTIVATION_PROGRESS_V3"
RECEIPT_VERSION = "US_RECORDED_PARTY_HISTORY_ACTIVATION_RECEIPT_V2"
...
def load_plan(path: Path, expected_sha: str) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("US recorded party V3 activation plan SHA mismatch")
    plan = dict(envelope["plan"])
    if plan.get("version") != PLAN_VERSION:
        raise RuntimeError("US recorded party V3 activation plan version is unsupported")
    return plan
...
def validate_live_plan(
    ...
    *,
    ...
    if sha != str(plan["expected_main"]) or sha != str(plan["implementation_sha"]):
        raise RuntimeError("US recorded party V3 activation main SHA drifted")
...
def load_progress(
    ...
    if payload.get("version") != PROGRESS_VERSION:
        raise RuntimeError("US recorded party V3 progress version is unsupported")
    progress = ActivationProgress(**dict(payload["progress"]))
    if progress.plan_sha256 != plan_sha256:
        raise RuntimeError("US recorded party V3 progress belongs to another plan")
    return progress
...
def expected_batch_stats(
    ...
    *,
    ...
    if len(rows) != 1:
        raise RuntimeError("US recorded party V3 expected batch statistics failed")
    return {
...
def actual_batch_stats(
    ...
    *,
    ...
    if len(rows) != 1:
        raise RuntimeError("US recorded party V3 actual batch statistics failed")
    return {
        "row_count": int(rows[0][0] or 0),
        "row_hash_xor": int(rows[0][1] or 0),
    }
    ...
    *,
    ...
    try:
        ...
        if actual_total != expected_total:
            raise RuntimeError(
                "US recorded party V3 final completeness mismatch: "
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Observation-identity-safe serial-bounded US recorded party history activation"
    )
