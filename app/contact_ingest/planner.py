...
def _plan_entity(
    ...
    *,
    ...
    name = _first(values, "ENTITY_NAME")
    if not name:
        return None

    country_code = "CN" if profile == "QCC_COMPANY_EXPORT" else normalize_country_code(_first(values, "COUNTRY"))
    ...
    entity = EntityPlan(
        ...
        source_row=source_row,
        raw_record={header: _row_value(row, idx) for idx, header in enumerate(headers) if clean_text(header)},
    )

    credit_code = normalize_credit_code(_first(values, "CREDIT_CODE"))
    if credit_code:
        entity.identifiers["CN_USCC" if country_code == "CN" else "REGISTRATION_ID"] = credit_code
    agent_code = clean_text(_first(values, "AGENT_CODE")).upper()
    if agent_code:
        entity.identifiers["CN_AGENT_CODE" if country_code == "CN" else "AGENT_CODE"] = agent_code
    ...
    for field_name, language in (("FORMER_NAME", "zh"), ("ENGLISH_NAME", "en")):
        for _mapping, raw in values.get(field_name, []):
            for alias in split_values(raw):
                if normalized_match_text(alias) and normalized_match_text(alias) != entity.normalized_name:
    ...
    contact_name = _first(values, "CONTACT_PERSON")
    ...
    for canonical, declared_type in (
        ...
        ("PERSON_EMAIL", "EMAIL"), ("PERSON_PHONE", "PHONE"),
    ):
        for mapping, raw in values.get(canonical, []):
            for item in split_values(raw):
                ...
                profile_person = profile == "AGENT_CONTACT_LIST" and contact_person is not None and declared_type != "WEBSITE"
                owner_is_person = contact_person is not None and (explicit_person or profile_person)
...
def build_plan(path: Path, *, source_name: str = "") -> ImportPlan:
    ...
    for table in tables:
        ...
        if header_idx < 0 or header_score < 5.0:
            global_warnings.append(
                f"Skipped {table.source_member}:{table.sheet_name or '<table>'}; no reliable contact header row detected."
            )
            continue
        ...
        if profile == "UNKNOWN":
            global_warnings.append(
                f"Skipped {table.source_member}:{table.sheet_name or '<table>'}; source profile is UNKNOWN."
            )
            continue

        entities: list[EntityPlan] = []
        skipped = 0
        for zero_idx, row in enumerate(table.rows[header_idx + 1 :], start=header_idx + 1):
        ...
        if unknown_columns:
            ...
            warnings.append(f"Unmapped columns retained in raw_record: {preview}{suffix}")
    ...
    if not table_plans:
        raise ValueError("No ingestible entity/contact tables were detected in the input file")
