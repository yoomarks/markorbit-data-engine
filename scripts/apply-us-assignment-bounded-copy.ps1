...
$script:Issue=704
$script:ReceiptVersion='US_ASSIGNMENT_BOUNDED_COPY_EXECUTOR_V1'
$script:JournalVersion='US_ASSIGNMENT_BOUNDED_COPY_JOURNAL_V1'
$script:ReviewVersion='US_ASSIGNMENT_BOUNDED_COPY_REVIEW_V1'
...
$script:SupersededPlanSha='3b1005403e34ce3796ea006b0a99033668ce42ce7463e6b25335f0b617360273'
...
function Assert-SafeAssignmentTable([string]$Table){
    if($Table -notmatch '^us_assignment_[a-z0-9_]+$' -or $script:AssignmentTables -notcontains $Table){ throw "Unsafe Assignment table: $Table" }
}
function Resolve-ReviewPlan {
    ...
    $review=Read-JsonFile $reviewPath 'Assignment copy review receipt'
    if([string]$review.receipt_version -ne $script:ReviewVersion){ throw 'Review receipt version mismatch.' }
    if([string]$review.decision -ne 'US_ASSIGNMENT_BOUNDED_COPY_REVIEW_READY'){ throw 'Review receipt is not READY.' }
    if([string]$review.next_gate -ne 'BOUNDED_ASSIGNMENT_TABLE_COPY_EXECUTOR_IMPLEMENTATION'){ throw 'Review next gate drifted.' }
    if([bool]$review.copy_authorized -or [bool]$review.mutation_performed -or [bool]$review.credential_values_persisted){ throw 'Review safety state drifted.' }
    if([string]$review.accepted_connectivity_receipt_sha256 -ne $script:AcceptedConnectivityReceiptSha256){ throw 'Accepted connectivity receipt SHA drifted.' }
    ...
    $plan=Read-JsonFile $planPath 'Assignment copy plan'
    ...
    if($planSha -eq $script:SupersededPlanSha){ throw 'Historical pre-executor Assignment copy plan is superseded; rerun #701 on fresh main.' }
...
function Assert-ReviewedPlan([object]$Resolved){
    ...
    if([string]$p.accepted_connectivity.receipt_sha256 -ne $script:AcceptedConnectivityReceiptSha256){ throw 'Plan accepted connectivity receipt drifted.' }
    if([string]$p.scope -ne 'ASSIGNMENT_ONLY' -or [string]$p.checksum_contract.definition -ne $script:ChecksumDefinition){ throw 'Plan scope/checksum drifted.' }
    ...
    if($tables.Count -ne 4){ throw 'Reviewed Assignment table count drifted.' }
    $names=@($tables|ForEach-Object {[string]$_.table})
    if(($names -join '|') -ne ($script:AssignmentTables -join '|')){ throw 'Reviewed Assignment order drifted.' }
    for($i=0;$i -lt $tables.Count;$i++){
        $t=$tables[$i]; $name=[string]$t.table; Assert-SafeAssignmentTable $name
    ...
    if([bool]$p.constraints.copy_authorized -or [bool]$p.constraints.future_copy_authorized -or [bool]$p.constraints.ttab_copy_authorized){ throw 'Plan unexpectedly authorizes copy.' }
    return @($tables)
}
function Get-RequiredAuthorityToken([string]$PlanSha){ return "GO #704 US Assignment bounded table copy $PlanSha" }
...
function Get-SourceLogicalChecksum([string]$Table){
    Assert-SafeAssignmentTable $Table
...
function Get-SourceTableSnapshot([object]$PlanTable,[object]$Source){
    $table=[string]$PlanTable.table; Assert-SafeAssignmentTable $table
...
function Assert-TtabUntouched {
    $quoted=@('us_ttab_docket_history','us_ttab_party_history','us_ttab_proceeding_history','us_ttab_property_history'|ForEach-Object {"'$_'"}) -join ','
    $parts=@(Invoke-TargetRows "SELECT count() AS active_parts, coalesce(sum(rows),0) AS rows FROM system.parts WHERE database='$($script:Database)' AND active AND table IN ($quoted)" 'TTAB untouched state')
    if($parts.Count -ne 1 -or [int64]$parts[0].active_parts -ne 0 -or [int64]$parts[0].rows -ne 0){ throw 'TTAB target state is no longer empty; Assignment executor refuses to proceed.' }
}
...
function Invoke-AssignmentNativePipe([string]$Table,[string]$Gateway){
    Assert-SafeAssignmentTable $Table
    $user=Get-DotEnvValue 'CLICKHOUSE_USER'; $password=Get-DotEnvValue 'CLICKHOUSE_PASSWORD'
    $scriptText=Get-NativePipeScript $Table $Gateway
    if($scriptText -match '(?i)--password|us_ttab_'){ throw 'Native pipe script contains forbidden credential/TTAB argv text.' }
...
function Assert-PreCopyTable([object]$PlanTable,[object]$InitialSource,[object[]]$AllTables){
...
function Assert-PostCopyTable([object]$PlanTable,[object]$InitialSource,[object[]]$AllTables){
    ...
    return [ordered]@{
        ...
        capacity=$capacity;accepted_at=(Get-Date).ToUniversalTime().ToString('o')
    }
}
function Get-JournalPaths([string]$PlanSha){
    ...
    $dir=Join-Path $root "us_assignment_bounded_copy_$PlanSha"
...
function Assert-NoExistingJournal([string]$Path){
    if(Test-Path -LiteralPath $Path -PathType Leaf){ $old=Read-JsonFile $Path 'Existing copy journal'; throw "Assignment copy authority already consumed/frozen. state=$($old.state) journal=$Path" }
}
...
function New-CopyJournal([object]$Resolved,[object]$InitialSource,[string]$Token){
    return [ordered]@{
        ...
        source_remains_authoritative=$true;next_gate='US_ASSIGNMENT_BOUNDED_COPY_IN_PROGRESS'
    }
}
...
function Invoke-ContractFixture {
    ...
    Write-Host 'US_ASSIGNMENT_BOUNDED_COPY_EXECUTOR_CONTRACT_PASS'
...
try {
    Write-Host '===== US ASSIGNMENT BOUNDED COPY EXECUTOR ====='
    ...
    if([string]$sourceInitial.sha256 -ne [string]$resolved.plan.accepted_connectivity.source_identity_sha256){ throw 'Full source identity drifted from reviewed plan.' }
    ...
    Assert-TtabUntouched
    $preflights=@()
    foreach($tablePlan in $tables){ $preflights += Assert-PreCopyTable $tablePlan $sourceInitial $tables }
    Assert-ExactMain 'post-preflight'
    if($DryRun){
        $root=if([IO.Path]::IsPathRooted($EvidenceRoot)){[IO.Path]::GetFullPath($EvidenceRoot)}else{[IO.Path]::GetFullPath((Join-Path $repoRoot $EvidenceRoot))}
        $stamp=(Get-Date).ToUniversalTime().ToString('yyyyMMdd_HHmmss')
        $dir=Join-Path $root "us_assignment_bounded_copy_dryrun_$stamp"; New-Item -ItemType Directory -Force -Path $dir | Out-Null
        $receiptPath=Join-Path $dir 'dry_run_receipt.json'
        $receipt=[ordered]@{
            receipt_version=$script:ReceiptVersion;mode='DRY_RUN';decision='US_ASSIGNMENT_BOUNDED_COPY_READY_FOR_EXACT_GO'
            ...
            next_gate='EXACT_OPERATOR_GO_704_REQUIRED'
        }
        Write-JsonAtomic $receipt $receiptPath; Assert-ExactMain 'dry-run-exit'
        Write-Host 'decision=US_ASSIGNMENT_BOUNDED_COPY_READY_FOR_EXACT_GO'
    ...
    foreach($tablePlan in $tables){
        ...
        $pre=Assert-PreCopyTable $tablePlan $sourceInitial $tables
        ...
        Save-Journal $script:ActiveJournal $script:ActiveJournalPath
        Invoke-AssignmentNativePipe $table ([string]$pre.connectivity.gateway)
        $post=Assert-PostCopyTable $tablePlan $sourceInitial $tables
        $acceptance=[ordered]@{
            receipt_version='US_ASSIGNMENT_TABLE_COPY_ACCEPTANCE_V1'
            main_sha=$ExpectedMainSha.Trim().ToLowerInvariant();plan_sha256=$resolved.plan_sha256
            ...
            source_unchanged=$true;target_parts_hot_us_only=$true;credential_values_persisted=$false
        }
    ...
    if([string]$sourceFinal.sha256 -ne [string]$sourceInitial.sha256){ throw 'Authoritative source changed during Assignment copy.' }
    Assert-TtabUntouched
    if(@($script:ActiveJournal.accepted_tables).Count -ne 4){ throw 'Final accepted Assignment table count is not four.' }
    $finalStates=@()
    foreach($tablePlan in $tables){
    ...
    $script:ActiveJournal.next_gate='ASSIGNMENT_TARGET_ACCEPTANCE_AND_TTAB_REVIEW'
    Save-Journal $script:ActiveJournal $script:ActiveJournalPath
    $success=[ordered]@{
        receipt_version=$script:ReceiptVersion;decision='US_ASSIGNMENT_BOUNDED_COPY_SUCCESS'
        main_sha=$ExpectedMainSha.Trim().ToLowerInvariant();plan_sha256=$resolved.plan_sha256
        review_receipt_sha256=$resolved.review_sha256;authority_consumed=$true;copy_executed=$true
        copied_table_count=4;source_unchanged=$true;ttab_untouched=$true
        target_states=@($finalStates);capacity_after=$finalCapacity;journal_path=$script:ActiveJournalPath
        credential_values_persisted=$false;next_gate='ASSIGNMENT_TARGET_ACCEPTANCE_AND_TTAB_REVIEW'
    }
    Write-JsonAtomic $success $paths.success; Assert-ExactMain 'success-exit'
    Write-Host 'decision=US_ASSIGNMENT_BOUNDED_COPY_SUCCESS'
    Write-Host "plan_sha256=$($resolved.plan_sha256)"; Write-Host 'copied_table_count=4'
    Write-Host 'source_unchanged=True'; Write-Host 'ttab_untouched=True'
    Write-Host 'next_gate=ASSIGNMENT_TARGET_ACCEPTANCE_AND_TTAB_REVIEW'
    ...
    if($script:ActiveJournal -and $script:ActiveJournalPath -and (Test-Path -LiteralPath $script:ActiveJournalPath -PathType Leaf)){
        try {
            ...
            $script:ActiveJournal.target_assignment_state=@(Get-FailureTargetState)
            ...
            $script:ActiveJournal.next_gate='US_ASSIGNMENT_PARTIAL_COPY_REMEDIATION_REVIEW'
            Save-Journal $script:ActiveJournal $script:ActiveJournalPath
        } catch { Write-Host 'journal_freeze_error=[REDACTED_OR_NONCRITICAL]' }
    }
    Write-Host "US_ASSIGNMENT_BOUNDED_COPY_FAILED: $message"
