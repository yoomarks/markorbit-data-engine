...
function Get-DefaultWslDistro {
    ...
    $distroKey = Join-Path $root ([string]$rootItem.DefaultDistribution)
...
try {
    ...
    $freshState = [bool](-not $runtimeRegistered -and -not $runtimeRootExists -and -not $exportTarExists)
    $resumeState = [bool]($ResumeExisting -and $runtimeExact -and $runtimeRootExists -and $exportTarExists)
    $runtimeMode = if ($resumeState) { 'resume' } elseif ($freshState) { 'fresh' } else { 'blocked' }
    ...
    if ($ResumeExisting) {
        if (-not $runtimeRegistered) { $blockers += 'RESUME_RUNTIME_DISTRO_MISSING' }
        elseif (-not $runtimeExact) { $blockers += 'RESUME_RUNTIME_IDENTITY_MISMATCH' }
        if (-not $runtimeRootExists) { $blockers += 'RESUME_RUNTIME_ROOT_MISSING' }
        if (-not $exportTarExists) { $blockers += 'RESUME_EXPORT_TAR_MISSING' }
    }
    elseif (-not $freshState) {
        $blockers += 'SPIKE_RUNTIME_PARTIAL_STATE_REQUIRES_RESUME'
    }
    ...
    $preflightReady = [bool]($blockers.Count -eq 0 -and ($freshState -or $resumeState))
    $decision = if ($preflightReady -and $resumeState) { 'READY_FOR_DEDICATED_WSL_CLICKHOUSE_RESUME' } elseif ($preflightReady) { 'READY_FOR_DEDICATED_WSL_CLICKHOUSE_APPLY' } else { 'WSL_CLICKHOUSE_SPIKE_BLOCKED' }
    ...
    if ($Apply) {
        ...
        try {
            if ($runtimeMode -eq 'fresh') {
                ...
                $runtimeStage = 'resume_existing_runtime'
            }

            $runtimeProbe = Invoke-RuntimeShell 'printf RUNTIME_OK' -AllowFailure
            if ($runtimeProbe['exit_code'] -ne 0 -or ((@($runtimeProbe['lines']) -join '').Trim() -ne 'RUNTIME_OK')) { throw 'Dedicated runtime distro did not start successfully.' }

            $runtimeStage = 'install_clickhouse'
            $installCommand = "set -eu; curl -fL --retry 3 --connect-timeout 15 '$packageUrl' -o '$packagePath'; sha256sum '$packagePath'; if clickhouse client --version 2>/dev/null | grep -F '$ClickHouseVersion' >/dev/null; then echo PACKAGE_INSTALL_SKIPPED_EXACT_VERSION; else dpkg -i '$packagePath' >/tmp/markorbit-clickhouse-dpkg.log 2>&1; echo PACKAGE_INSTALL_PERFORMED; fi; clickhouse client --version"
            ...
            foreach ($line in @($install['lines'])) {
                ...
                if ($line.Trim() -eq 'PACKAGE_INSTALL_PERFORMED') { $packageInstallPerformed = $true }
            }
            if (-not $packageSha256) { throw 'ClickHouse package SHA256 was not captured.' }
...
"@
            ...
            if ($serverStarted) { $serverStopped = Stop-SpikeServer }
            if ($CleanupMounts) {
    ...
    $report = [ordered]@{
        receipt_version = 'DEDICATED_WSL_CLICKHOUSE_SPIKE_V2'
        decision = $decision
        engine_sha = $ExpectedMainSha.Trim().ToLowerInvariant()
        apply_requested = [bool]$Apply
        resume_existing_requested = [bool]$ResumeExisting
    ...
    Write-Host "resume_existing_requested=$([bool]$ResumeExisting)"
