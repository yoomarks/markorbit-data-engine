...
$script:SupersededChecksumV1EngineSha = '55bd797274c5d678a8ab6f7d1262458eea9fcf62'
$script:ChecksumReceiptVersion = 'PRODUCTION_CN_WARM_SOURCE_LOGICAL_CHECKSUM_V2'
$script:ChecksumJournalVersion = 'PRODUCTION_CN_WARM_SOURCE_LOGICAL_CHECKSUM_JOURNAL_V2'
...
$script:V1RowHashExpression = 'cityHash64(tuple(*))'
$script:V2RowHashExpression = 'cityHash64(toJSONString(tuple(*)))'
...
function Assert-DesignV1LogicalSql([string]$Sql, [object]$Plan, [object]$Unit) {
    ...
    if ($Sql -notmatch $pattern) { throw "Accepted V1 design logical checksum SQL changed for $($Plan.table)/$($Unit.partition_id)." }
    $matches = [regex]::Matches($Sql, [regex]::Escape($script:V1RowHashExpression))
    if ($matches.Count -ne 2) { throw "Accepted V1 design must contain exactly two frozen row-hash expressions for $($Plan.table)/$($Unit.partition_id)." }
}

function ConvertTo-V2LogicalSql([string]$DesignV1Sql, [object]$Plan, [object]$Unit) {
    Assert-DesignV1LogicalSql $DesignV1Sql $Plan $Unit
    $v2 = $DesignV1Sql.Replace($script:V1RowHashExpression, $script:V2RowHashExpression)
    if ($v2 -eq $DesignV1Sql) { throw 'V2 checksum derivation made no change.' }
    if ($v2.Contains($script:V1RowHashExpression)) { throw 'V2 checksum derivation left a V1 row-hash expression behind.' }
    $v2Matches = [regex]::Matches($v2, [regex]::Escape($script:V2RowHashExpression))
    if ($v2Matches.Count -ne 2) { throw 'V2 checksum derivation did not produce exactly two NULL-safe row-hash expressions.' }
    return $v2
}
...
function Invoke-LogicalChecksum([string]$ExecutionQuery, [object]$Plan, [object]$Unit) {
    ...
    if ($probe.exit_code -ne 0) { throw "V2 logical checksum query failed for $($Plan.table)/$($Unit.partition_id): $($probe.lines -join [Environment]::NewLine)" }
    $lines = @($probe.lines | Where-Object { $_.Trim() })
    if ($lines.Count -ne 1) { throw "V2 logical checksum returned $($lines.Count) rows for $($Plan.table)/$($Unit.partition_id)." }
    $fields = @($lines[0] -split "`t", -1)
    if ($fields.Count -ne 3) { throw "V2 logical checksum returned unexpected field count for $($Plan.table)/$($Unit.partition_id)." }
    if ($fields[0] -notmatch '^\d+$' -or $fields[1] -notmatch '^\d+$' -or $fields[2] -notmatch '^\d+$') { throw 'V2 logical checksum output must be exact unsigned decimal strings.' }
    if ([int64]$fields[0] -ne [int64]$Unit.rows) { throw "V2 logical checksum row count mismatch for $($Plan.table)/$($Unit.partition_id)." }
...
function Invoke-ContractFixture {
    ...
    $v1 = "SELECT count() AS rows, sum(cityHash64(tuple(*))) AS checksum_sum, groupBitXor(cityHash64(tuple(*))) AS checksum_xor FROM markorbit_facts.cn_observed_event WHERE _partition_id = 'all'"
    $v2 = ConvertTo-V2LogicalSql $v1 $fixture $unit
    if ($v2 -notmatch 'cityHash64\(toJSONString\(tuple\(\*\)\)\)' -or $v2 -match 'cityHash64\(tuple\(\*\)\)') { throw 'V2 NULL-safe checksum derivation contract failed.' }
    $execution = Get-ExecutionQuery $v2
    if ($execution -notmatch 'max_threads = 2' -or $execution -notmatch 'FORMAT TabSeparatedRaw') { throw 'Bounded V2 checksum execution query contract failed.' }
    $sample = [ordered]@{ table='cn_observed_event'; partition_id='all'; rows='10'; checksum_sum='18446744073709551615'; checksum_xor='9223372036854775808'; design_v1_logical_sql_sha256='a'; logical_sql_v2_sha256='b'; execution_query_sha256='c'; source_identity_sha256='d' }
    if ((Get-UnitResultSha $sample) -notmatch '^[0-9a-f]{64}$') { throw 'V2 checksum result canonical hash contract failed.' }
    Write-Host 'PRODUCTION_CN_WARM_SOURCE_LOGICAL_CHECKSUM_V2_CONTRACT_DIRECT_INVOCATION_OK'
}

try {
    Write-Host '===== PRODUCTION CN WARM SOURCE LOGICAL CHECKSUM V2 ====='
    Write-Host "checksum_definition=$script:ChecksumDefinition"
    Write-Host 'null_safe_row_serialization=toJSONString(tuple(*))'
    ...
    if ((git branch --show-current).Trim() -ne 'main') { throw 'Source logical checksum V2 gate must run from local main.' }
    ...
    if (-not [bool]$production.ready) { throw 'Production ClickHouse must be healthy before V2 logical checksum scans.' }
    ...
    if ($ResumeEvidenceDirectory) {
        ...
        $evidenceDir = Join-Path $repoRoot (Join-Path $EvidenceRoot "production_cn_warm_source_logical_checksum_v2_$timestamp")
        New-Item -ItemType Directory -Force -Path $evidenceDir | Out-Null
    }
    Write-Host "Evidence directory: $evidenceDir"
    $journalPath = Join-Path $evidenceDir 'production_cn_warm_source_logical_checksum_v2_journal.json'

    if (Test-Path -LiteralPath $journalPath -PathType Leaf) {
        $journal = Read-JsonFile $journalPath 'V2 checksum journal'
        if ([string]$journal.receipt_version -ne $script:ChecksumJournalVersion) { throw 'Unexpected V2 checksum journal version.' }
        if ([string]$journal.engine_sha -ne $ExpectedMainSha.Trim().ToLowerInvariant()) { throw 'V2 checksum journal main SHA changed.' }
        if ([string]$journal.design_receipt_sha256 -ne $acceptedDesign.sha256) { throw 'V2 checksum journal design receipt SHA changed.' }
        if ([string]$journal.checksum_definition -ne $script:ChecksumDefinition) { throw 'V2 checksum journal definition changed.' }
        if ([int]$journal.max_threads -ne $MaxThreads -or [int]$journal.max_execution_seconds -ne $MaxExecutionSeconds) { throw 'V2 checksum journal execution settings changed.' }
        ...
        $journal = [ordered]@{
            ...
            supersedes_failed_v1_engine_sha=$script:SupersededChecksumV1EngineSha
    ...
    foreach ($plan in $acceptedDesign.plans) {
        foreach ($unit in @($plan.partitions)) {
            ...
            $designV1Sql = [string]$unit.logical_checksum_sql
            Assert-DesignV1LogicalSql $designV1Sql $plan $unit
            $logicalSqlV2 = ConvertTo-V2LogicalSql $designV1Sql $plan $unit
            $designV1SqlSha = Get-StringSha256 $designV1Sql
            $logicalSqlV2Sha = Get-StringSha256 $logicalSqlV2
            $executionQuery = Get-ExecutionQuery $logicalSqlV2
            $executionQuerySha = Get-StringSha256 $executionQuery

            Write-Host "checksum_unit_start=$unitKey"
            Write-Host "checksum_query_version=v2|unit=$unitKey|design_v1_sql_sha256=$designV1SqlSha|logical_sql_v2_sha256=$logicalSqlV2Sha"
            ...
            if ($existing.Count -gt 1) { throw "Duplicate V2 checksum journal entries for $unitKey." }
            if ($existing.Count -eq 1) {
                if ([string]$existing[0].design_v1_logical_sql_sha256 -ne $designV1SqlSha -or [string]$existing[0].logical_sql_v2_sha256 -ne $logicalSqlV2Sha -or [string]$existing[0].execution_query_sha256 -ne $executionQuerySha -or [string]$existing[0].source_identity_sha256 -ne $sourceIdentity) { throw "V2 resume identity drift for $unitKey." }
                if ([string]$existing[0].result_sha256 -ne (Get-UnitResultSha $existing[0])) { throw "V2 resume result hash drift for $unitKey." }
            ...
            if ($afterIdentity -ne $sourceIdentity) { throw "Source identity changed during V2 checksum scan for $unitKey." }
            ...
            $record = [ordered]@{
                ...
                checksum_definition=$script:ChecksumDefinition
                design_v1_logical_sql_sha256=$designV1SqlSha
                logical_sql_v2_sha256=$logicalSqlV2Sha
    ...
    if ($results.Count -ne $expectedUnitCount) { throw "V2 checksum result count mismatch: expected $expectedUnitCount observed $($results.Count)." }
    $checksumRows = [int64](($results | ForEach-Object { [int64]$_.rows } | Measure-Object -Sum).Sum)
    if ($checksumRows -ne $script:ExpectedWarmRows) { throw 'Final V2 checksum row total changed.' }
    $manifestLines = @($results | ForEach-Object { "$($_.table)|$($_.partition_id)|$($_.rows)|$($_.checksum_sum)|$($_.checksum_xor)|$($_.design_v1_logical_sql_sha256)|$($_.logical_sql_v2_sha256)|$($_.execution_query_sha256)|$($_.source_identity_sha256)|$($_.result_sha256)|$script:ChecksumDefinition" })
    $checksumManifestSha = Get-StringSha256 ($manifestLines -join "`n")

    Assert-RawConsumersStopped
    $finalHealth = Get-ProductionClickHouseHealth
    if (-not [bool]$finalHealth.ready) { throw 'Production ClickHouse must remain healthy after V2 logical checksum scans.' }
    Assert-AcceptedProductionMount $finalHealth.container_id
    Assert-ExactMain 'final'
    if ((Get-FileSha256 $envPath) -ne $envShaBefore) { throw '.env changed during V2 logical checksum gate.' }
    ...
    if (Test-Path -LiteralPath $script:ExpectedWarmVhdxPath) { throw 'Production Warm VHDX was created during read-only V2 checksum gate.' }

    $receipt = [ordered]@{
        receipt_version=$script:ChecksumReceiptVersion
        engine_sha=$ExpectedMainSha.Trim().ToLowerInvariant()
        supersedes_failed_v1_engine_sha=$script:SupersededChecksumV1EngineSha
        decision='PRODUCTION_CN_WARM_SOURCE_LOGICAL_CHECKSUM_V2_READY_FOR_REVIEW'
        ...
        checksum_execution=[ordered]@{
            row_serialization='toJSONString(tuple(*))'
            row_hash='cityHash64(toJSONString(tuple(*)))'
            aggregate_sum='sum(row_hash)'
            aggregate_xor='groupBitXor(row_hash)'
            null_safe=$true
            max_threads=$MaxThreads
            max_execution_seconds=$MaxExecutionSeconds
    ...
    $receiptPath = Join-Path $evidenceDir 'production_cn_warm_source_logical_checksum_v2.json'
    Write-JsonFile $receipt $receiptPath
    Assert-ExactMain 'exit'

    Write-Host '===== PRODUCTION CN WARM SOURCE LOGICAL CHECKSUM V2 RESULT ====='
    Write-Host 'decision=PRODUCTION_CN_WARM_SOURCE_LOGICAL_CHECKSUM_V2_READY_FOR_REVIEW'
    ...
    foreach ($result in $results) { Write-Host "checksum_unit=$($result.table)|partition=$($result.partition_id)|rows=$($result.rows)|sum=$($result.checksum_sum)|xor=$($result.checksum_xor)|v2_sql_sha256=$($result.logical_sql_v2_sha256)|result_sha256=$($result.result_sha256)" }
    Write-Host 'read_only=True'
    Write-Host 'mutation_performed=False'
    ...
    Write-Host "Evidence directory: $evidenceDir"
    Write-Host 'PRODUCTION_CN_WARM_SOURCE_LOGICAL_CHECKSUM_V2_DONE'
    exit 0
}
catch {
    Write-Host "PRODUCTION_CN_WARM_SOURCE_LOGICAL_CHECKSUM_V2_FAILED: $($_.Exception.Message)"
    exit 2
}
