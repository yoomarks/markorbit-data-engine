...
try {
    ...
    if ($workerId) {
        throw "Persistent worker is running. Stop it before CN observed-event compaction: docker compose stop worker"
    }

    $pythonArgs = @(
        "-m", "app.cn.storage_v2_event_compaction",
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "CN observed-event Storage V2 compaction failed with exit code $LASTEXITCODE"
    }
}
finally {
    Pop-Location
