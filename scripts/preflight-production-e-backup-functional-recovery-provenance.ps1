...
param(
    ...
    [Parameter(Mandatory = $true)][string]$AcceptedEBackupPreflightReceiptPath,
...
function Get-ProcessBackupReferences([string]$BackupRoot) {
    ...
    foreach ($process in @(Get-CimInstance Win32_Process -ErrorAction Stop)) {
        ...
        if ($commandLine -and $commandLine.IndexOf($needle, [System.StringComparison]::OrdinalIgnoreCase) -ge 0) {
            $refs += [pscustomobject]@{ source='process_command_line'; identity="$([string]$process.Name):$([int]$process.ProcessId)"; path=$needle }
        }
    }
    return @($refs)
}
...
function Assert-ToolingProvenance {
    ...
    if ($changed.Count -ne 3 -or $unexpected.Count -ne 0 -or $missing.Count -ne 0) { throw 'Functional provenance escaped the exact 3-file tooling boundary.' }
}
...
function Resolve-AcceptedPreflightReceipt {
    ...
    if ([bool]$receipt.e_backup.delete_authorized -or [bool]$receipt.duplicate_identity.proven) { throw 'Accepted preflight unexpectedly authorized/proved E deletion.' }
...
function Get-DiskImageEvidence([string]$Path) {
    if ($null -eq (Get-Command Get-DiskImage -ErrorAction SilentlyContinue)) { return [ordered]@{ proof_available=$false; attached=$null; reason='Get-DiskImage unavailable' } }
    try {
...
function Get-VhdMetadataReadOnly([string]$Path) {
    $result = [ordered]@{
        path=$Path; get_vhd_available=$false; metadata_ready=$false; vhd_format=$null; vhd_type=$null;
        file_size=$null; virtual_size=$null; minimum_size=$null; logical_sector_size=$null; physical_sector_size=$null;
        block_size=$null; parent_path=$null; disk_identifier=$null; fragmentation_percentage=$null; attached=$null; reason=$null
    }
    if ($null -eq (Get-Command Get-VHD -ErrorAction SilentlyContinue)) { $result.reason='Get-VHD unavailable'; return $result }
...
function Get-FileEvidence([object]$File, [string]$Role, [string]$CurrentRoot) {
    $info = New-Object System.IO.FileInfo([string]$File.path)
    $currentPath = Join-Path $CurrentRoot ([string]$File.relative_path)
    $currentExists = Test-Path -LiteralPath $currentPath -PathType Leaf
    $current = $null
    if ($currentExists) {
        $currentInfo = New-Object System.IO.FileInfo($currentPath)
        $currentAttr = [System.IO.File]::GetAttributes($currentPath)
        $currentReparse = [bool](($currentAttr -band [System.IO.FileAttributes]::ReparsePoint) -ne 0)
        $current = [ordered]@{
            path=$currentInfo.FullName; length=[int64]$currentInfo.Length;
            allocated_bytes=if ($currentReparse) { [int64]0 } else { Get-AllocatedFileBytes $currentInfo.FullName };
            last_write_utc=$currentInfo.LastWriteTimeUtc.ToString('o'); reparse=$currentReparse;
            disk_image=(Get-DiskImageEvidence $currentInfo.FullName); vhd=(Get-VhdMetadataReadOnly $currentInfo.FullName)
        }
    }
    return [ordered]@{
        relative_path=[string]$File.relative_path; path=$info.FullName; role=$Role; length=[int64]$info.Length;
        allocated_bytes=[int64]$File.allocated_bytes; creation_utc=$info.CreationTimeUtc.ToString('o'); last_write_utc=$info.LastWriteTimeUtc.ToString('o');
        reparse=$false; current_counterpart_exists=$currentExists; current_counterpart=$current;
        disk_image=(Get-DiskImageEvidence $info.FullName); vhd=(Get-VhdMetadataReadOnly $info.FullName)
    }
}

function Get-SmallFilePreview([string]$Path) {
    $info = New-Object System.IO.FileInfo($Path); $preview=$null
...
function Compare-VhdLineage([object]$Left,[object]$Right) {
    ...
    $leftId=[string]$Left.disk_identifier; $rightId=[string]$Right.disk_identifier
    if ($leftId -and $rightId -and $leftId.Equals($rightId,[System.StringComparison]::OrdinalIgnoreCase)) { return [ordered]@{ state='SAME_VIRTUAL_DISK_IDENTIFIER'; reason='DiskIdentifier equal' } }
    $shapeEqual=[bool]([string]$Left.vhd_format -eq [string]$Right.vhd_format -and [int64]$Left.virtual_size -eq [int64]$Right.virtual_size -and [int64]$Left.logical_sector_size -eq [int64]$Right.logical_sector_size -and [int64]$Left.physical_sector_size -eq [int64]$Right.physical_sector_size -and [int64]$Left.block_size -eq [int64]$Right.block_size)
...
function Invoke-ContractFixture {
    foreach ($name in @('Assert-ExactMain','Normalize-HostPath','Test-PathsOverlap','Get-OptionalArrayProperty','Get-AllContainerMounts','Get-ComposeBindMounts','Get-WslBasePaths','Get-BackupReferenceInventory','Get-DirectoryInventoryNoFollowAllocated','Get-AllocatedFileBytes')) {
        if ($null -eq (Get-Command $name -ErrorAction SilentlyContinue)) { throw "Required helper missing: $name" }
    }
    $fakeEnv=@{}; $refs=@(Get-BackupReferenceInventory 'C:\fixture\backup' @() @() $fakeEnv)
    if ($refs.Count -ne 0) { throw 'Backup reference helper closure fixture failed.' }
    ...
    if ((Get-BackupRole 'disk\docker_data.empty.vhdx') -ne 'docker_data_empty_placeholder_snapshot') { throw 'empty role fixture failed.' }
    if ((Get-BackupRole 'main\ext4.vhdx') -ne 'docker_desktop_system_distro_snapshot') { throw 'system role fixture failed.' }
    ...
    if ((Compare-VhdLineage $left $right).state -ne 'SAME_VIRTUAL_DISK_IDENTIFIER') { throw 'VHD lineage fixture failed.' }
...
try {
    ...
    Write-Host 'read_only=True'; Write-Host 'mutation_performed=False'; Write-Host 'e_backup_delete_authorized=False'
    Write-Host 'raw_delete_authorized=False'; Write-Host 'accepted_volume_mutation_authorized=False'; Write-Host 'docker_restart_authorized=False'
    Write-Host 'docker_prune_authorized=False'; Write-Host 'wsl_mutation_authorized=False'; Write-Host 'vhdx_mutation_authorized=False'
    Write-Host 'clickhouse_mutation_authorized=False'; Write-Host 'cn_warm_move_authorized=False'; Write-Host 'us_bulk_authorized=False'
    ...
    Assert-ExactMain 'entry'; Assert-ToolingProvenance
    $admin=New-Object Security.Principal.WindowsPrincipal([Security.Principal.WindowsIdentity]::GetCurrent())
    if (-not $admin.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) { throw 'Functional provenance review requires elevated Administrator PowerShell.' }

    $eBackup=Normalize-HostPath $EBackupRoot; $currentRoot=Normalize-HostPath $CurrentDockerDesktopWslRoot; $fRecovery=Normalize-HostPath $FRecoveryRoot; $expectedF=Normalize-HostPath $ExpectedFRecoveryVhdx
    ...
    Write-Host "accepted_e_backup_preflight_receipt=$($accepted.path)"; Write-Host "accepted_e_backup_preflight_receipt_sha256=$($accepted.sha256)"
    $envPath=Join-Path $repoRoot '.env'; $envShaBefore=if(Test-Path -LiteralPath $envPath -PathType Leaf){Get-Sha256 $envPath}else{$null}
    Assert-ExactMain 'review_before'; $beforeBoundary=Assert-ProductionBoundary 'before'

    $inventory=Get-DirectoryInventoryNoFollowAllocated $eBackup
    if (-not [bool]$inventory.exists -or [int64]$inventory.reparse_count -ne 0) { throw 'E backup disappeared or gained reparse points.' }
    if ([int64]$inventory.file_count -ne [int64]$accepted.receipt.e_backup.file_count -or [int64]$inventory.vhdx_count -ne [int64]$accepted.receipt.e_backup.vhdx_count -or [int64]$inventory.logical_bytes -ne [int64]$accepted.receipt.e_backup.logical_bytes -or [int64]$inventory.allocated_bytes -ne [int64]$accepted.receipt.e_backup.allocated_bytes) { throw 'E backup inventory changed from accepted preflight.' }

    $vhdxEvidence=@(); $unresolvedRoleCount=0; $counterpartMissingCount=0
    foreach($file in @($inventory.vhdx)){
        $role=Get-BackupRole $file.relative_path; if($role -eq 'unresolved'){$unresolvedRoleCount++}
        $evidence=Get-FileEvidence $file $role $currentRoot; if(-not [bool]$evidence.current_counterpart_exists){$counterpartMissingCount++}
        ...
        if($evidence.current_counterpart){Write-Host "d_counterpart relative_path=$($evidence.relative_path) bytes=$($evidence.current_counterpart.length) allocated_bytes=$($evidence.current_counterpart.allocated_bytes) attached=$($evidence.current_counterpart.disk_image.attached) get_vhd_ready=$($evidence.current_counterpart.vhd.metadata_ready) disk_identifier=$($evidence.current_counterpart.vhd.disk_identifier) virtual_size=$($evidence.current_counterpart.vhd.virtual_size) vhd_type=$($evidence.current_counterpart.vhd.vhd_type)"}
    }

    $smallFileEvidence=@(); foreach($file in @($inventory.files|Where-Object{-not [bool]$_.is_vhdx})){
        $small=Get-SmallFilePreview $file.path; $smallFileEvidence += [pscustomobject]$small
        $preview=if($small.utf8_preview){([string]$small.utf8_preview).Replace("`r",' ').Replace("`n",' ')}else{''}
        Write-Host "e_non_vhdx path=$($small.path) bytes=$($small.length) sha256=$($small.sha256) preview=$preview"
    }

    if(-not(Test-Path -LiteralPath $expectedF -PathType Leaf)){throw 'Expected F recovery VHDX disappeared.'}
    $fInfo=New-Object System.IO.FileInfo($expectedF); $fAttr=[System.IO.File]::GetAttributes($expectedF); if(($fAttr -band [System.IO.FileAttributes]::ReparsePoint)-ne 0){throw 'F recovery VHDX became reparse.'}
    $fDiskImage=Get-DiskImageEvidence $expectedF; $fVhd=Get-VhdMetadataReadOnly $expectedF
    ...
    $eDockerData=@($vhdxEvidence|Where-Object{$_.role -eq 'docker_data_primary_snapshot'}); $lineage=[ordered]@{state='UNRESOLVED';reason='Expected exactly one E docker_data primary snapshot.'}
    if($eDockerData.Count -eq 1){$lineage=Compare-VhdLineage $eDockerData[0].vhd $fVhd}
    Write-Host "e_f_docker_data_lineage_state=$($lineage.state)"; Write-Host "e_f_docker_data_lineage_reason=$($lineage.reason)"

    $expectedRoleSetReady=[bool](@($vhdxEvidence|Where-Object{$_.role -eq 'docker_data_primary_snapshot'}).Count -eq 1 -and @($vhdxEvidence|Where-Object{$_.role -eq 'docker_data_empty_placeholder_snapshot'}).Count -eq 1 -and @($vhdxEvidence|Where-Object{$_.role -eq 'docker_desktop_system_distro_snapshot'}).Count -eq 1)
    $allDetachedProofReady=[bool](@($vhdxEvidence|Where-Object{-not [bool]$_.disk_image.proof_available -or [bool]$_.disk_image.attached}).Count -eq 0 -and [bool]$fDiskImage.proof_available -and -not [bool]$fDiskImage.attached)
    $currentCounterpartsReady=[bool]($counterpartMissingCount -eq 0)
    Write-Host "e_vhdx_expected_role_set_ready=$expectedRoleSetReady"; Write-Host "e_vhdx_unresolved_role_count=$unresolvedRoleCount"; Write-Host "e_vhdx_current_counterpart_missing_count=$counterpartMissingCount"; Write-Host "e_and_f_vhdx_detached_proof_ready=$allDetachedProofReady"

    $inventoryAfter=Get-DirectoryInventoryNoFollowAllocated $eBackup
    $inventoryStable=[bool]([string]$inventoryAfter.metadata_manifest_sha256 -eq [string]$inventory.metadata_manifest_sha256 -and [int64]$inventoryAfter.file_count -eq [int64]$inventory.file_count -and [int64]$inventoryAfter.logical_bytes -eq [int64]$inventory.logical_bytes -and [int64]$inventoryAfter.allocated_bytes -eq [int64]$inventory.allocated_bytes)
    Write-Host "e_backup_inventory_stable=$inventoryStable"; if(-not $inventoryStable){throw 'E backup inventory changed during provenance review.'}

    Assert-ExactMain 'review_final'; $finalBoundary=Assert-ProductionBoundary 'final'; $envShaAfter=if(Test-Path -LiteralPath $envPath -PathType Leaf){Get-Sha256 $envPath}else{$null}; $envUnchanged=[bool]($envShaBefore -eq $envShaAfter); Write-Host "env_unchanged=$envUnchanged"
    $decision='PRODUCTION_E_BACKUP_FUNCTIONAL_RECOVERY_PROVENANCE_BLOCKED'; $nextGate='PRODUCTION_E_BACKUP_PRESERVE_AND_REPLAN'
    if($expectedRoleSetReady -and $currentCounterpartsReady -and $allDetachedProofReady -and $inventoryStable -and $envUnchanged){$decision='PRODUCTION_E_BACKUP_FUNCTIONAL_RECOVERY_PROVENANCE_READY_FOR_OPERATOR_DECISION';$nextGate='PRODUCTION_E_BACKUP_RECLAIM_OPERATOR_ACK_REVIEW'}

    $timestamp=Get-Date -Format 'yyyyMMdd_HHmmss'; $evidenceDir=Join-Path $repoRoot (Join-Path $EvidenceRoot "production_e_backup_functional_recovery_provenance_$timestamp"); New-Item -ItemType Directory -Force -Path $evidenceDir|Out-Null
    $receipt=[ordered]@{
        receipt_version=$script:ReceiptVersion; engine_sha=$ExpectedMainSha.Trim().ToLowerInvariant(); decision=$decision; next_gate=$nextGate; read_only=$true; mutation_performed=$false;
        accepted_e_backup_preflight_receipt_path=$accepted.path; accepted_e_backup_preflight_receipt_sha256=$accepted.sha256;
        e_backup=[ordered]@{root=$eBackup;file_count=[int64]$inventory.file_count;vhdx_count=[int64]$inventory.vhdx_count;logical_bytes=[int64]$inventory.logical_bytes;allocated_bytes=[int64]$inventory.allocated_bytes;metadata_manifest_sha256=$inventory.metadata_manifest_sha256;inventory_stable=$inventoryStable;expected_role_set_ready=$expectedRoleSetReady;current_counterparts_ready=$currentCounterpartsReady;all_detached_proof_ready=$allDetachedProofReady;vhdx=@($vhdxEvidence);non_vhdx=@($smallFileEvidence);delete_authorized=$false};
        f_recovery=[ordered]@{path=$expectedF;bytes=[int64]$fInfo.Length;allocated_bytes=[int64](Get-AllocatedFileBytes $expectedF);disk_image=$fDiskImage;vhd=$fVhd};
        docker_data_lineage=$lineage; production_invariant_preserved=[bool]($beforeBoundary.production.ready -and $finalBoundary.production.ready); env_unchanged=$envUnchanged;
        constraints=[ordered]@{e_backup_delete_authorized=$false;raw_delete_authorized=$false;accepted_volume_mutation_authorized=$false;docker_restart_authorized=$false;docker_prune_authorized=$false;wsl_mutation_authorized=$false;vhdx_mutation_authorized=$false;clickhouse_mutation_authorized=$false;cn_warm_move_authorized=$false;us_bulk_authorized=$false}
    }
    $receiptPath=Join-Path $evidenceDir 'production_e_backup_functional_recovery_provenance.json'; $receipt|ConvertTo-Json -Depth 16|Set-Content -LiteralPath $receiptPath -Encoding UTF8
    Assert-ExactMain 'exit'
    Write-Host '===== PRODUCTION E BACKUP FUNCTIONAL RECOVERY PROVENANCE RESULT ====='; Write-Host "decision=$decision"; Write-Host "next_gate=$nextGate"; Write-Host 'read_only=True'; Write-Host 'mutation_performed=False'; Write-Host "expected_role_set_ready=$expectedRoleSetReady"; Write-Host "current_counterparts_ready=$currentCounterpartsReady"; Write-Host "all_detached_proof_ready=$allDetachedProofReady"; Write-Host "docker_data_lineage_state=$($lineage.state)"; Write-Host 'e_backup_delete_authorized=False'; Write-Host 'cn_warm_move_authorized=False'; Write-Host 'vhdx_mutation_authorized=False'; Write-Host "receipt_path=$receiptPath"; Write-Host "Evidence directory: $evidenceDir"; Write-Host 'PRODUCTION_E_BACKUP_FUNCTIONAL_RECOVERY_PROVENANCE_DONE'
}
