...
CREATE INDEX IF NOT EXISTS ix_contact_import_run_started
ON contact.import_run(started_at DESC);
...
CREATE INDEX IF NOT EXISTS ix_contact_person_name
ON contact.person(normalized_name);
...
CREATE INDEX IF NOT EXISTS ix_contact_relation_entity
ON contact.entity_person_relation(entity_id, relation_type);
...
ON contact.channel(entity_id, channel_type, normalized_value)
WHERE entity_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS ux_contact_channel_person
ON contact.channel(person_id, channel_type, normalized_value)
WHERE person_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS ix_contact_channel_normalized
ON contact.channel(channel_type, normalized_value);
...
CREATE INDEX IF NOT EXISTS ix_contact_raw_record_entity
ON contact.raw_record(entity_id, source_id);
...
CREATE INDEX IF NOT EXISTS ix_contact_observation_channel
ON contact.channel_observation(channel_id, observed_at DESC);
...
person_owner AS (
    SELECT DISTINCT ON (person_id)
        person_id, entity_id, relation_type, title, department
    FROM contact.entity_person_relation
    ORDER BY person_id, last_seen_at DESC
)
SELECT
    e.entity_id,
    ...
    e.city,
    p.person_id,
    p.canonical_name AS contact_person,
    po.relation_type,
    po.title,
    po.department,
    c.channel_id,
...
VALUES ('CONTACT_INGEST', 'CONTACT_SCHEMA_V1')
